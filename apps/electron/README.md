# opentomo Electron App

The primary desktop interface for opentomo, built with Electron + React. Provides a multi-session inbox with chat interface for interacting with Claude via opentomo workspaces.
...
│   │   ├── sessions.ts    # Session management, opentomoAgent integration
...
The `AgentEvent` types from `opentomoAgent` use different property names than you might expect:
...
### 4. opentomoAgent Constructor

`opentomoAgent` expects the full `Workspace` object, not just the ID:

```typescript
// Wrong:
new opentomoAgent({ workspaceId: workspace.id, model })

// Correct:
new opentomoAgent({ workspace, model })
```

### 5. esbuild Configuration

The supported build entrypoint is the root script:

```bash
bun run electron:build
```

That script fans out to dedicated TypeScript build helpers for main, preload, renderer,
resources, assets, and SDK staging.

For the **main process**, we intentionally keep a curated bundling policy in sync across
both production builds and `electron:dev`:
- Runtime-sensitive packages are externalized (`electron`, `better-sqlite3`, `node-pty`,
  `ai`, `@ai-sdk/google`, `zod`, `wsl-utils`, `powershell-utils`)
- A small Bun-managed alias list is resolved from `node_modules/.bun` for packages whose
  transitive dependencies do not bundle reliably through Bun symlinks
- If a new Bun-managed package enters the main bundle and fails in CI or Windows dev,
  either externalize it or add it to the explicit Bun alias list

The preload build remains intentionally simple and only externalizes `electron`.
...
**How it works:**
- `.env.1password` contains `op://` references to your 1Password vault
...
```bash
bun run electron:build           # Supported CI/release build entrypoint
bun run electron:build:main      # Bundle main process via scripts/electron-build-main.ts
bun run electron:build:preload   # Bundle preload script via scripts/electron-build-preload.ts
bun run electron:build:renderer  # Bundle React app (Vite)
bun run electron:build:resources # Copy icons/resources
bun run electron:stage-deps      # Stage Bun-managed runtime deps for electron-builder
bun run electron:dev             # Dev/watch build; must stay in sync with main bundle policy
```
...
- **Subagent support** - Load and apply agent definitions from opentomo documents
...
External apps can navigate using `opentomo://` URLs:

```
opentomo://settings
opentomo://allChats/chat/session123
opentomo://sources/source/github
opentomo://action/new-chat
opentomo://workspace/{id}/allChats/chat/abc123
```
...
| `main/sessions.ts` | opentomoAgent wrapper, event processing, source integration |
