...
echo "=== Building opentomo AppImage (${ARCH}) using electron-builder ==="
if [ "$UPLOAD" = true ]; then
...
cp "$TEMP_DIR/node-${NODE_VERSION}-linux-${NODE_ARCH}/bin/node" "$ELECTRON_DIR/vendor/node/node"
...
# electron-builder outputs: opentomo-x86_64.AppImage or opentomo-aarch64.AppImage
BUILT_APPIMAGE_NAME="opentomo-${LINUX_ARCH}.AppImage"
...
# Rename to our standard naming convention: opentomo-x64.AppImage, opentomo-arm64.AppImage
APPIMAGE_NAME="opentomo-${ARCH}.AppImage"
