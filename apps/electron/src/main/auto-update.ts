 ...
 * (OpenTomo/opentomo-app-release repository).
 *
 * Platform behavior:
...
import { getAppVersion } from '@opentomo/shared/version'
import {
...
// Periodic update check interval (every 4 hours)
let __updateCheckInterval: NodeJS.Timeout | null = null
...
/**
 * Error category for update check failures
 */
...
/**
 * Categorize update check errors into user-friendly types
 */
function categorizeUpdateError(error: unknown, isTimeout: boolean): CategorizedError {
  if (isTimeout) {
    return {
      category: 'timeout',
      userMessage: 'Update check timed out. This may be due to network policy restrictions blocking access to GitHub.',
      technicalMessage: 'Update check exceeded 30 second timeout',
    }
  }
  ...
  // DNS resolution failure (specific network issue)
  if (errorCode === 'ENOTFOUND') {
    return {
      category: 'dns_failure',
      userMessage: 'Cannot resolve update server address. Check your DNS settings or network configuration.',
      technicalMessage: `DNS lookup failed: ${errorMessage}`,
    }
  }

  // Network unreachable errors
  const networkErrorCodes = ['ETIMEDOUT', 'ECONNREFUSED', 'ENETDOWN', 'ENETUNREACH', 'EHOSTDOWN', 'EHOSTUNREACH', 'EPIPE']
  if (errorCode && networkErrorCodes.includes(errorCode)) {
    return {
      category: 'network_unreachable',
      userMessage: 'Cannot reach update server. Check your internet connection, firewall, or proxy settings.',
      technicalMessage: `Network error (${errorCode}): ${errorMessage}`,
    }
  }

  // GitHub blocked (403, authentication issues)
  if (errorMessage.includes('403') || errorMessage.includes('Forbidden') || errorMessage.includes('blocked')) {
    return {
      category: 'github_blocked',
      userMessage: 'Access to GitHub releases blocked. Your network policy may restrict update checks.',
      technicalMessage: `GitHub access forbidden: ${errorMessage}`,
    }
  }
  ...
  return {
    category: 'generic',
    userMessage: 'Failed to check for updates. Please try again later or check your network connection.',
 ...
 * Returns { result, timedOut } where timedOut indicates if timeout won
 */
async function withTimeout<T>(
  ...
  const wrappedPromise = promise.then(
    (result) => ({ result, timedOut: false as const }),
    (error) => {
  ...
  try {
    const outcome = await Promise.race([wrappedPromise, timeoutPromise])
...
autoUpdater.on('update-available', (info) => {
  ...
  // Only set 'downloading' if a download will actually start
  const willDownload = autoUpdater.autoDownload
  updateInfo = {
    ...updateInfo,
    available: true,
    latestVersion: info.version,
    downloadState: willDownload ? 'downloading' : 'idle',
    downloadProgress: 0,
  }
  broadcastUpdateInfo()
})
...
autoUpdater.on('error', (error) => {
  const categorized = categorizeUpdateError(error, false)
  mainLog.error(`[auto-update] Error (${categorized.category}):`, categorized.technicalMessage)
...
export async function checkForUpdates(options: CheckOptions = {}): Promise<UpdateInfo> {
  ...
  try {
    ...
    mainLog.info('[auto-update] Starting update check with 30s timeout...')
    ...
    if (timedOut) {
      ...
      const categorized = categorizeUpdateError(null, true)
      mainLog.warn(`[auto-update] ${categorized.technicalMessage}`)
    ...
    // Success - existing logic continues
    ...
    mainLog.error(`[auto-update] Check failed (${categorized.category}):`, categorized.technicalMessage)
 ...
 * Checks in background and auto-downloads if update available.
 * Call this after app initialization.
 */
export function startPeriodicUpdateChecks(): void {
  if (__updateCheckInterval) {
    mainLog.warn('[auto-update] Periodic checks already running')
    return
  }

  mainLog.info(`[auto-update] Starting periodic checks (every ${UPDATE_CHECK_INTERVAL / (60 * 60 * 1000)} hours)`)

  __updateCheckInterval = setInterval(async () => {
    if (!app.isReady()) return

    mainLog.info('[auto-update] Running periodic check...')
    try {
      const info = await checkForUpdates({ autoDownload: true })

      // If periodic check resulted in error, log it but don't show notification
      // (user shouldn't be bothered by network issues during background checks)
      if (info.downloadState === 'error') {
        mainLog.warn(`[auto-update] Periodic check failed silently: ${info.error}`)
      }
    } catch (error) {
      // This shouldn't happen since checkForUpdates catches its errors,
      // but handle it just in case
      mainLog.error('[auto-update] Periodic check threw unexpected error:', error)
    }
 ...
 * Call this during app shutdown cleanup.
 */
export function stopPeriodicUpdateChecks(): void {
  if (__updateCheckInterval) {
    clearInterval(__updateCheckInterval)
    __updateCheckInterval = null
    mainLog.info('[auto-update] Stopped periodic checks')
  }
}
