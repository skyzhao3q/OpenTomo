 ...
 * Deep Link Handler
 *
 * Parses opentomo:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   opentomo://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   opentomo://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   opentomo://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   opentomo://sources[/source/{sourceSlug}]          - Sources list
 *   opentomo://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   opentomo://action/{actionName}[/{id}][?params]
 *   opentomo://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   opentomo://allChats                               (all chats view)
 *   opentomo://allChats/chat/abc123                   (specific chat)
 *   opentomo://settings/shortcuts                     (shortcuts page)
 *   opentomo://sources/source/github                  (github source info)
 *   opentomo://action/new-chat                        (uses active window)
 *   opentomo://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   opentomo://workspace/ws123/allChats/chat/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., opentomo://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., opentomo://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // opentomo://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // opentomo://allChats/..., opentomo://settings/..., etc. (compound routes)
    ...
    // opentomo://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // opentomo://action/... (no workspace - uses active window)
    if (host === 'action') {
