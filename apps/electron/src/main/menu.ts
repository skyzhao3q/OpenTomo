...
import { getAgentName } from '@opentomo/shared/prompts/soul'
 ...
 * Includes only relevant items for the OpenTomo app.
 *
 ...
 *
 * On Windows/Linux: Menu is hidden - all functionality is in the OpenTomo logo menu.
...
export async function rebuildMenu(): Promise<void> {
  ...
  // Users access menu via the OpenTomo logo dropdown in the app
  if (!isMac) {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    ...(isMac ? [{
      label: agentName,
      submenu: [
        { role: 'about' as const, label: `About ${agentName}` },
        updateMenuItem,
        ...
        { type: 'separator' as const },
        { role: 'hide' as const, label: `Hide ${agentName}` },
        { role: 'hideOthers' as const },
        { role: 'unhide' as const },
        { type: 'separator' as const },
        { role: 'quit' as const, label: `Quit ${agentName}` }
      ]
    }] : []),
