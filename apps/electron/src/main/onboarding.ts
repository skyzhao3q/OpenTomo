...
import { getAuthState, getSetupNeeds } from '@opentomo/shared/auth'
import { getCredentialManager } from '@opentomo/shared/credentials'
import { saveConfig, loadStoredConfig, generateWorkspaceId, type AuthType, type StoredConfig } from '@opentomo/shared/config'
import { getDefaultWorkspacesDir, generateUniqueWorkspacePath, isValidWorkspace, createWorkspaceAtPath, ensureWorkspaceDirStructure } from '@opentomo/shared/workspaces'
import { startClaudeOAuth, exchangeClaudeCode, hasValidOAuthState, clearOAuthState } from '@opentomo/shared/auth'
import { getCredentialManager as getCredentialManagerFn } from '@opentomo/shared/credentials'
import {
  IPC_CHANNELS,
...
export function registerOnboardingHandlers(sessionManager: SessionManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.ONBOARDING_SAVE_CONFIG, async (_event, config: {
    ...
    try {
      ...
      // 5a. Ensure workspace folder structures exist on disk immediately after saving config.
      // Without this, folder creation is deferred to the next loadStoredConfig() call,
      // which may fail silently and leave workspaces in a broken state (New Chat would silently fail).
      for (const ws of newConfig.workspaces) {
        try {
          if (!isValidWorkspace(ws.rootPath)) {
            mainLog.info('[Onboarding:Main] Creating workspace folder:', ws.rootPath)
            createWorkspaceAtPath(ws.rootPath, ws.name)
            mainLog.info('[Onboarding:Main] Workspace folder created successfully:', ws.rootPath)
          } else {
            ensureWorkspaceDirStructure(ws.rootPath)
          }
        } catch (folderErr) {
          mainLog.error('[Onboarding:Main] Failed to create workspace folder:', ws.rootPath, folderErr)
        }
      }
