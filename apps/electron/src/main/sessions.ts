...
import { OpenTomoAgent, type AgentEvent, setPermissionMode, type PermissionMode, unregisterSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, setAnthropicOptionsEnv, setPathToClaudeCodeExecutable, setInterceptorPath, setExecutable } from '@opentomo/shared/agent'
...
import {
  ...
  getModel,
  getToolIconsDir,
} from '@opentomo/shared/config'
import { loadWorkspaceConfig } from '@opentomo/shared/workspaces'
import {
...
} from '@opentomo/shared/sessions'
import { getAuthState } from '@opentomo/shared/auth'
import { registerBundledRuntime } from '@opentomo/shared/runtimes'
import { getCredentialManager } from '@opentomo/shared/credentials'
import { type Session, type Message, type SessionEvent, type FileAttachment, type StoredAttachment, type SendMessageOptions, IPC_CHANNELS, generateMessageId } from '../shared/types'
import { regenerateSessionTitle, formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrl, getEmojiIcon, resetSummarizationClient, resolveToolIcon } from '@opentomo/shared/utils'
import { loadWorkspaceSkills, type LoadedSkill } from '@opentomo/shared/skills'
import type { ToolDisplayMeta } from '@opentomo/core/types'
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL } from '@opentomo/shared/agent/thinking-levels'
...
function resolveToolDisplayMeta(
  ...
  // MCP source tools have no icon (sources feature removed)
  ...
  // and resolves their brand icon from ~/.opentomo/tool-icons/
...
interface ManagedSession {
  id: string
  workspace: Workspace
  agent: OpenTomoAgent | null  // Lazy-loaded - null until first message
  messages: Message[]
  ...
  // pendingTextParent) has been removed. OpenTomoAgent now provides parentToolUseId
  ...
  // Token refresh manager removed - no longer needed for local-only sources
  // One-shot retry flag for title generation when proxy auth isn't ready yet
  titleRetryScheduled?: boolean
}
...
export class SessionManager {
  ...
  // Last warmup timestamp (ms) to throttle repeated warmup calls
  ...
  /**
   * Fire a minimal API request to warm up the provider connection.
   * Only runs for custom base URL providers (Azure, OpenRouter, etc.) where cold starts
   * are observed. No-op for direct Anthropic. Throttled to once per
   * 2 minutes. Never throws — warmup failure is non-critical.
   */
  async warmupApiConnection(): Promise<void> {
    // Only warm up custom-URL providers (Azure, OpenRouter, etc.)
    const baseUrl = process.env.ANTHROPIC_BASE_URL
    if (!baseUrl || baseUrl.startsWith('https://api.anthropic.com')) {
      return  // Direct Anthropic — no warmup needed
    }

    // Skip if auth token is missing or still pending
    const authToken = process.env.ANTHROPIC_AUTH_TOKEN
    if (!authToken || authToken === 'not-needed') {
      return
    }

    // Throttle: skip if last warmup was less than 2 minutes ago
    const now = Date.now()
    if (now - this.lastWarmupAt < 2 * 60 * 1000) {
      return
    }
    this.lastWarmupAt = now

    try {
      sessionLog.info('[Warmup] Warming up custom API provider:', baseUrl)
      ...
      // AbortController with 15s timeout — warmup is best-effort
      ...
      try {
        ...
        sessionLog.info('[Warmup] Provider warmed up successfully')
      } finally {
      ...
      if (!msg.toLowerCase().includes('abort')) {
        sessionLog.info('[Warmup] Warmup request failed (non-critical):', msg.slice(0, 200))
      }
    }
  }
  ...
  setupConfigWatcher(workspaceRootPath: string, workspaceId: string): void {
    ...
    const callbacks: ConfigWatcherCallbacks = {
      ...
      onSkillChange: async (slug, skill) => {
        ...
        const { loadWorkspaceSkills } = await import('@opentomo/shared/skills')
        const skills = loadWorkspaceSkills(workspaceRootPath)
        this.broadcastSkillsChanged(skills)
      },
      onCommandsListChange: async (commands) => {
        sessionLog.info(`Commands list changed in ${workspaceRootPath} (${commands.length} commands)`)
        this.broadcastCommandsChanged(commands)
      },
      onCommandChange: async (slug, command) => {
        sessionLog.info(`Command '${slug}' changed:`, command ? 'updated' : 'deleted')
        // Broadcast updated list to UI
        const { loadAllCommands } = await import('@opentomo/shared/commands')
        const commands = loadAllCommands(workspaceRootPath)
        this.broadcastCommandsChanged(commands)
      },

      // Session metadata changes (external edits to session.jsonl headers).
      // Detects label/flag/name changes made by other instances or scripts.
   ...
   */
  private broadcastAppThemeChanged(theme: import('@opentomo/shared/config').ThemeOverrides | null): void {
   ...
   */
  private broadcastSkillsChanged(skills: LoadedSkill[]): void {
    if (!this.windowManager) return
  ...
  /**
   * Broadcast commands changed event to all windows
   */
  private broadcastCommandsChanged(commands: import('@opentomo/shared/commands').LoadedCommand[]): void {
    if (!this.windowManager) return
    sessionLog.info(`Broadcasting commands changed (${commands.length} commands)`)
    this.windowManager.broadcastToAll(IPC_CHANNELS.COMMANDS_CHANGED, commands)
  }

  /**
   * Broadcast default permissions changed event to all windows
   * Triggered when ~/.opentomo/permissions/default.json changes
   */
  ...
  /**
   * Build connector MCP server state for injection into the agent.
   * Connectors feature has been removed; returns empty state.
   */
  private async buildConnectorServerState(_workspaceRootPath: string): Promise<{
    apiServers: Record<string, unknown>
    virtualSources: unknown[]
    intendedSlugs: string[]
  }> {
    return { apiServers: {}, virtualSources: [], intendedSlugs: [] }
  }

  /**
   * Reload sources for a session with an active agent.
   * Sources feature has been removed; no-op.
   */
  private async reloadSessionSources(_managed: ManagedSession): Promise<void> {
   ...
   * See: https://github.com/OpenTomo/opentomo-org/issues/39
   */
  async reinitializeAuth(): Promise<void> {
    try {
      ...
      if (customBaseUrl && !customBaseUrl.startsWith('https://api.anthropic.com')) {
        // Priority 1: Custom third-party URL (Ollama, OpenRouter, Vercel, etc.)
        // Use Bearer auth (ANTHROPIC_AUTH_TOKEN) for third-party providers — this matches
        // the test-connection handler (ipc.ts) which uses authToken for custom URLs.
        // OpenRouter requires Authorization: Bearer for correct model routing;
        // x-api-key (ANTHROPIC_API_KEY) causes "model not found" for non-Claude models.
        // NOTE: https://api.anthropic.com is excluded here — it falls through to Priority 3
        // (ANTHROPIC_API_KEY) so the SDK sends x-api-key instead of Authorization: Bearer.
        process.env.ANTHROPIC_BASE_URL = customBaseUrl
        delete process.env.CLAUDE_CODE_OAUTH_TOKEN
        delete process.env.ANTHROPIC_API_KEY  // prevent SDK from also sending x-api-key
        // Strip Azure AI Foundry overrides — if set, they cause the SDK CLI to route
        // to Azure even when ANTHROPIC_BASE_URL points to a different provider (e.g. Ollama).
        ...
        if (billing.apiKey) {
          ...
          // For custom_api connections: the global anthropic_api_key is a copy made by
          // setActiveConnection(), which isn't called on restart. Read the connection-
          // specific key directly as a fallback.
          const connectionId = getActiveConnectionId()
          if (connectionId) {
            const connectionKey = await getCredentialManager().getConnectionApiKey(connectionId)
            if (connectionKey) {
              process.env.ANTHROPIC_AUTH_TOKEN = connectionKey
              sessionLog.info(`Using connection API key for ${customBaseUrl}`)
            } else {
              process.env.ANTHROPIC_AUTH_TOKEN = 'not-needed'
              sessionLog.warn('Custom base URL configured but no API key set. Using placeholder token (works for Ollama, will fail for OpenRouter).')
            }
          } else {
            // Set a placeholder token for providers like Ollama that don't validate keys
            process.env.ANTHROPIC_AUTH_TOKEN = 'not-needed'
            sessionLog.warn('Custom base URL configured but no API key set. Using placeholder token (works for Ollama, will fail for OpenRouter).')
          }
        }
      ...
      // Log effective env vars so we can verify routing in debug output
      sessionLog.info(`[reinitializeAuth] ANTHROPIC_BASE_URL=${process.env.ANTHROPIC_BASE_URL ?? '(not set)'}, hasAuthToken=${!!process.env.ANTHROPIC_AUTH_TOKEN}, hasApiKey=${!!process.env.ANTHROPIC_API_KEY}`)
  ...
  /**
   * Invalidate all agent instances so they are recreated with fresh env vars
   * on the next message. Call this after switching provider connections.
   * The SDK subprocess has env vars baked in at creation — we must destroy it
   * and let it restart with the updated ANTHROPIC_BASE_URL / auth token.
   */
  invalidateAllAgents(): void {
    for (const [, managed] of this.sessions) {
      if (managed.agent) {
        managed.agent = null
        // Clear SDK session ID: sessions are provider-specific and cannot be
        // resumed across different providers (proxy vs Ollama have incompatible auth).
        managed.sdkSessionId = undefined
        sessionLog.info(`Agent invalidated for session ${managed.id} after connection change`)
      }
    }
  }

  async initialize(): Promise<void> {
    // Load sessions from disk FIRST, before any async auth operations.
    // This eliminates the race condition where the renderer calls getSessions()
    // before loadSessionsFromDisk() has run (renderer auth check can complete
    // in ~200-1000ms while reinitializeAuth() may take 50-200ms on Windows).
    // Sessions are read-only from disk and require no credentials.
    this.loadSessionsFromDisk()

    // Set path to Claude Code executable (cli.js from SDK)
    // In packaged app: compute correct base paths depending on whether asar is enabled.
    //   - asar disabled: app.getAppPath() → Resources/app/   (all files under one dir)
    //   - asar enabled:  app.getAppPath() → Resources/app.asar
    //       extraResources (to: app/...)  → Resources/app/         (use dirname + 'app')
    //       asarUnpack files              → Resources/app.asar.unpacked/ (use dirname + 'app.asar.unpacked')
    // In development: use process.cwd()
    const appPath = app.getAppPath()
    const isAsar = app.isPackaged && appPath.endsWith('.asar')
    // SDK is in extraResources (to: app/node_modules/...) → always at Resources/app/
    const basePath = app.isPackaged
      ? (isAsar ? join(dirname(appPath), 'app') : appPath)
      : process.cwd()
    // asarUnpack files (interceptor, bun, node) → Resources/app.asar.unpacked/ when asar enabled
    const unpackedBase = app.isPackaged
      ? (isAsar ? join(dirname(appPath), 'app.asar.unpacked') : appPath)
      : process.cwd()
    ...
    // In packaged app: the file is in asarUnpack → Resources/app.asar.unpacked/packages/...
    // In monorepos (dev): packages may be at the root level, not inside apps/electron
    const interceptorRelativePath = join('packages', 'shared', 'src', 'network-interceptor.ts')
    let interceptorPath = join(unpackedBase, interceptorRelativePath)
    if (!existsSync(interceptorPath) && !app.isPackaged) {
      // Try monorepo root (../../packages from apps/electron)
      const monorepoRoot = join(unpackedBase, '..', '..')
    ...
    if (app.isPackaged) {
      ...
      // On macOS/Linux, bun is in asarUnpack (unpackedBase). See electron-builder.yml for details.
      const bunBasePath = process.platform === 'win32' ? process.resourcesPath : unpackedBase
      const bunPath = join(bunBasePath, 'vendor', 'bun', bunBinary)
      if (!existsSync(bunPath)) {
        const error = [
          `Bundled Bun runtime not found at ${bunPath}.`,
          '',
          'Possible causes:',
          '  - Antivirus or Windows Defender may have quarantined the file',
          '  - The app installation may be corrupted',
          '',
          'To fix this:',
          '  1. Try reinstalling the application',
          '  2. Add the app installation directory to your antivirus exclusions',
          '  3. Check Windows Defender quarantine for blocked files',
        ].join('\n')
        sessionLog.error(error)
        throw new Error(error)
      }
      sessionLog.info('[Info] Using bundled Bun at:', bunPath)
      setExecutable(bunPath)
      registerBundledRuntime('bun', bunPath)

      // Resolve and register bundled Node.js LTS runtime
      const nodeBinary = process.platform === 'win32' ? 'node.exe' : 'node'
      const nodeBasePath = process.platform === 'win32' ? process.resourcesPath : unpackedBase
      const nodePath = join(nodeBasePath, 'vendor', 'node', nodeBinary)
      if (existsSync(nodePath)) {
        sessionLog.info('[Info] Using bundled Node.js at:', nodePath)
        registerBundledRuntime('node', nodePath)
      } else {
        sessionLog.warn('[Warning] Bundled Node.js not found at', nodePath, '— will use system node')
      }
    }
    // In development: use system 'bun' / 'node' (already on PATH in terminal-launched dev mode)
  ...
  private loadSessionsFromDisk(): void {
    try {
      ...
      // Notify all renderer windows that sessions are ready.
      // Defense-in-depth: if a renderer called getSessions() before this ran
      // (e.g. due to a future timing change), it can re-fetch on receiving this event.
      BrowserWindow.getAllWindows().forEach(w => {
        if (!w.webContents.isDestroyed()) {
          try { w.webContents.send(IPC_CHANNELS.SESSIONS_INITIALIZED) } catch { /* ignore */ }
        }
      })
    } catch (error) {
  ...
  private getAuthRequestDescription(request: AuthRequest): string {
    // Only credential auth is supported for local sources
   ...
   * Note: OAuth flows are no longer supported - only credential auth for local sources
   */
  private async runOAuthFlow(managed: ManagedSession, request: AuthRequest): Promise<void> {
    ...
    // OAuth is not supported for local-only sources
    sessionLog.error(`OAuth flow requested but not supported for local sources: ${request.sourceSlug}`)
    await this.completeAuthRequest(managed.id, {
      requestId: request.requestId,
      sourceSlug: request.sourceSlug,
      success: false,
      error: `OAuth authentication is not supported`,
    })
  }
  ...
  async createSession(workspaceId: string, options?: import('../shared/types').CreateSessionOptions): Promise<Session> {
    ...
    // Read permission mode from workspace config, fallback to app setting, then global defaults
    ...
    // 1. Workspace model — but only if user explicitly set it (≠ auto-default DEFAULT_MODEL)
    // 2. Global config model — from App Settings "Default Chat Model"
    // 3. undefined — renderer falls back to App.tsx currentModel state
    const workspaceConfigModel = wsConfig?.defaults?.model
    const globalConfigModel = getModel()
    const defaultModel = (workspaceConfigModel && workspaceConfigModel !== DEFAULT_MODEL)
    ...
    // Assign new non-hidden sessions to the "Uncategorized" project by default
    const defaultProjectId = options?.hidden ? undefined : 'uncategorized'
    if (defaultProjectId) {
      updateSessionMetadata(workspaceRootPath, storedSession.id, { projectId: defaultProjectId })
    }
    ...
    // Prewarm: create agent in background so it's ready when user sends first message.
    // This moves agent creation + system prompt build off the critical path.
    // Only for non-hidden sessions (hidden = inline/mini agents that send immediately).
    if (!options?.hidden) {
      this.getOrCreateAgent(managed).catch((err) => {
        sessionLog.warn(`Agent prewarm failed for session ${managed.id}:`, err)
      })
    }
   ...
   */
  private async getOrCreateAgent(managed: ManagedSession): Promise<OpenTomoAgent> {
    if (!managed.agent) {
      const end = perf.start('agent.create', { sessionId: managed.id })
      const config = loadStoredConfig()
      managed.agent = new OpenTomoAgent({
        ...
        // Connectors feature removed; no connector IDs to pass
        connectorIds: [],
        // Pass active auth type so agent can generate provider-appropriate error messages
        authType: getAuthType() ?? undefined,
        ...
        // Pass disabled skill slugs so agent blocks calls to these skills
        disabledSkillSlugs: (() => {
          try {
            const { getWorkspaceDisabledCustomSkills } = require('@opentomo/shared/workspaces') as typeof import('@opentomo/shared/workspaces')
            return getWorkspaceDisabledCustomSkills(managed.workspace.rootPath)
          } catch { return [] }
        })(),
      })
      ...
      // The legacy onCredentialRequest callback has been removed from OpenTomoAgent
      ...
      // Wire up onSourceActivationRequest - sources feature removed, always returns false
      managed.agent.onSourceActivationRequest = async (_sourceSlug: string): Promise<boolean> => {
        return false
      }
      ...
      // Always call setPermissionMode (even for undefined) so onPermissionModeChange fires
      // and the renderer receives the correct mode via IPC.
      const effectiveMode = managed.permissionMode ?? getDefaultChatMode()
      setPermissionMode(managed.id, effectiveMode)
      sessionLog.info(`Applied permission mode '${effectiveMode}' to agent for session ${managed.id}`)
      end()
    }
   ...
   * Sources feature has been removed; this is a no-op stub kept for API compatibility.
   */
  async setSessionSources(sessionId: string, _sourceSlugs: string[]): Promise<void> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      throw new Error(`Session not found: ${sessionId}`)
    }
    // Sources feature removed — no-op
    sessionLog.info(`setSessionSources called for session ${sessionId} (sources feature removed)`)
  }

  /**
   * Get the enabled source slugs for a session.
   * Sources feature has been removed; always returns empty array.
   */
  getSessionSources(_sessionId: string): string[] {
    return []
  }
  ...
  async sendMessage(sessionId: string, message: string, attachments?: FileAttachment[], storedAttachments?: StoredAttachment[], options?: SendMessageOptions, existingMessageId?: string, _isAuthRetry?: boolean): Promise<void> {
    ...
    // - Clear pending plan execution (file I/O)
    // - Ensure messages are loaded (file I/O)
    ...
    // Reset first-delta tracking for immediate flush of first token
    this.firstDeltaSent.delete(managed.id)
    ...
    sendSpan.mark('pre_processing.done')

    // Get or create the agent (lazy loading)
    const agent = await this.getOrCreateAgent(managed)
    sendSpan.mark('agent.ready')

    // Sources feature has been removed — no source servers to apply
    ...
    try {
      ...
      // in the rawText, and canUseTool in opentomo-agent.ts provides a fallback
      // to qualify short names. No transformation needed here.

      // Design Agent mode has been removed along with media/prompt module
      let finalMessage = message

      sendSpan.mark('chat.starting')
      const chatIterator = agent.chat(finalMessage, attachments, _isAuthRetry)
      ...
      if (isAbortError) {
        ...
        console.error('[chat] Unhandled error in session', sessionId, error)
  ...
  private processNextQueuedMessage(sessionId: string): void {
    ...
    setImmediate(() => {
      this.sendMessage(
        ...
        console.error('[chat-queue] Error processing queued message for session', sessionId, err)
        this.sendEvent({
          type: 'error',
          sessionId,
...
• For ongoing shells, the agent can use BashOutput to check status`
  ...
  /**
   * Set the project for a session (null = uncategorized).
   * Projects are exclusive containers (one-per-session).
   */
  ...
  /**
   * Migrate all sessions with null/undefined projectId to 'uncategorized'.
   * Updates both in-memory sessions (lightweight — only metadata, no full persist)
   * and disk-only sessions. Sends project_changed events so the renderer re-groups.
   */
  migrateNullProjectsToUncategorized(workspaceId: string, workspaceRootPath: string): void {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { listSessions, updateSessionMetadata } = require('@opentomo/shared/sessions') as typeof import('@opentomo/shared/sessions')
    const inMemoryIds = new Set<string>()

    // Update in-memory sessions (lightweight update + metadata write, no full JSONL persist)
    for (const managed of this.sessions.values()) {
      if (managed.workspace.id === workspaceId && !managed.projectId && !managed.hidden) {
        managed.projectId = 'uncategorized'
        updateSessionMetadata(workspaceRootPath, managed.id, { projectId: 'uncategorized' })
        this.sendEvent({ type: 'project_changed', sessionId: managed.id, projectId: 'uncategorized' }, workspaceId)
        inMemoryIds.add(managed.id)
      }
    }

    // Update disk-only sessions not currently loaded in memory
    const sessions = listSessions(workspaceRootPath)
    for (const session of sessions) {
      if (!session.projectId && !session.hidden && !inMemoryIds.has(session.id)) {
        updateSessionMetadata(workspaceRootPath, session.id, { projectId: 'uncategorized' })
        this.sendEvent({ type: 'project_changed', sessionId: session.id, projectId: 'uncategorized' }, workspaceId)
      }
    }
  }

  /**
   * Batch-set projectId for multiple sessions in a single operation.
   * Handles both in-memory sessions (update + persist + send event) and
   * disk-only sessions (direct updateSessionMetadata + send event).
   * Used by the Smart Clean IPC handler.
   */
  batchSetSessionProjects(
    assignments: Array<{ sessionId: string; projectId: string }>,
    workspaceId: string,
    workspaceRootPath: string,
  ): void {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { updateSessionMetadata } = require('@opentomo/shared/sessions') as typeof import('@opentomo/shared/sessions')
    for (const { sessionId, projectId } of assignments) {
      ...
      if (managed) {
        ...
        // Session not in memory — update disk directly
        updateSessionMetadata(workspaceRootPath, sessionId, { projectId })
      }
      // Always send event so renderer updates session grouping
      this.sendEvent({ type: 'project_changed', sessionId, projectId }, workspaceId)
    }
  }

  /**
   * Update the title of a single session.
   * For in-memory sessions: updates name, persists, and sends title_generated event.
   * For disk-only sessions: writes name to disk via updateSessionMetadata.
   * Used by the Smart Clean IPC handler during batch title generation.
   */
  updateSessionTitle(sessionId: string, title: string, workspaceId: string, workspaceRootPath: string): void {
    const managed = this.sessions.get(sessionId)
    if (managed) {
      managed.name = title
      this.persistSession(managed)
      ...
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { updateSessionMetadata } = require('@opentomo/shared/sessions') as typeof import('@opentomo/shared/sessions')
      updateSessionMetadata(workspaceRootPath, sessionId, { name: title })
    }
  }

  /**
   * Clear projectId from all in-memory sessions that reference a deleted project.
   * Called when a project is deleted via IPC to keep in-memory state consistent.
   */
  clearProjectFromSessions(projectId: string): void {
    for (const [, managed] of this.sessions) {
      if (managed.projectId === projectId) {
        managed.projectId = null
      }
    }
  }
  ...
  private processEvent(managed: ManagedSession, event: AgentEvent): void {
    ...
    switch (event.type) {
      ...
      case 'text_complete': {
        ...
        managed.streamingText = ''
        // Reset first-delta tracking so next text segment gets immediate flush
        this.firstDeltaSent.delete(managed.id)
      ...
      case 'tool_start': {
        ...
        if (formattedToolInput && Object.keys(formattedToolInput).length > 0) {
          toolDisplayMeta = resolveToolDisplayMeta(event.toolName, formattedToolInput, workspaceRootPath)
        }
        ...
        // Use parentToolUseId directly from the event — OpenTomoAgent resolves this
      ...
      case 'tool_result': {
        // toolName comes directly from OpenTomoAgent (resolved via ToolIndex)
        ...
        // parentToolUseId comes from OpenTomoAgent (SDK-authoritative) or existing message
        ...
        if (existingToolMsg) {
          ...
          const fallbackToolDisplayMeta = resolveToolDisplayMeta(toolName, undefined, fallbackWorkspaceRootPath)
      ...
      case 'typed_error':
        ...
        if (isAuthError && !managed.authRetryAttempted && managed.lastSentMessage) {
          ...
          setImmediate(async () => {
            try {
              // 1. Refresh auth — oauth_token / api_key: standard reinitializeAuth (refreshes OAuth if needed)
              ...
              // Clear the SDK session ID so the retry starts a fresh session
              // instead of resuming the one that failed with auth error.
              // An auth failure may leave the session in an invalid state,
              // causing resume attempts to time out or fail again.
              if (managed.sdkSessionId) {
                sessionLog.info(`[auth-retry] Clearing SDK session ID to force fresh session`)
                managed.sdkSessionId = undefined
              }
              ...
              console.error('[auth-retry] Failed to retry after auth refresh for session', sessionId, retryError)
      ...
      case 'complete':
        // Complete event from OpenTomoAgent - accumulate usage from this turn
   ...
   */
  // Track whether first delta for a turn has been sent (reset when streaming starts)
  private firstDeltaSent: Set<string> = new Set()
  ...
  private queueDelta(sessionId: string, workspaceId: string, delta: string, turnId?: string): void {
    ...
    // Flush the first text_delta of a turn immediately for faster perceived response time
    // Subsequent deltas use the batched timer to reduce IPC overhead
    if (!this.firstDeltaSent.has(sessionId)) {
      this.firstDeltaSent.add(sessionId)
      this.flushDelta(sessionId, workspaceId)
      return
    }
