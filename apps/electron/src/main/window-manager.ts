...
export class WindowManager {
  ...
  createWindow(options: CreateWindowOptions): BrowserWindow {
    ...
    // Fallback: if ready-to-show never fires (e.g. GPU process crash, missing
    // libvk_swiftshader.dylib, renderer failure), force-show after timeout
    // so the user doesn't get an invisible app.
    let shown = false
    const showWindow = () => {
    ...
    setTimeout(() => {
      if (!shown && !window.isDestroyed()) {
        windowLog.warn('ready-to-show did not fire within 10s, force-showing window')
        showWindow()
      }
    }, 10_000)
