...
const api: ElectronAPI = {
  ...
  onSessionsInitialized: (callback: () => void) => {
    const handler = () => callback()
    ipcRenderer.on(IPC_CHANNELS.SESSIONS_INITIALIZED, handler)
    return () => {
      ipcRenderer.removeListener(IPC_CHANNELS.SESSIONS_INITIALIZED, handler)
    }
  },
  ...
  openAndReadFileAttachments: () => ipcRenderer.invoke(IPC_CHANNELS.OPEN_AND_READ_FILE_ATTACHMENTS),
  ...
  revealInExplorer: (path: string) => ipcRenderer.invoke(IPC_CHANNELS.REVEAL_IN_EXPLORER, path),
  ...
  getClaudeOAuthStatus: () => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_GET_CLAUDE_OAUTH_STATUS),
  activateClaudeOAuth: () => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_ACTIVATE_CLAUDE_OAUTH),
  disconnectClaudeOAuth: () => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_DISCONNECT_CLAUDE_OAUTH),
  ...
  getDefaultChatMode: () => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_GET_DEFAULT_CHAT_MODE),
  setDefaultChatMode: (mode: 'safe' | 'ask' | 'allow-all') => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_SET_DEFAULT_CHAT_MODE, mode),
  ...
  getProvider: () => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_GET_PROVIDER),
  setProvider: (provider: string) => ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_SET_PROVIDER, provider),
  ...
  }, apiKey: string) => ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_ADD, connectionData, apiKey),
  updateConnection: (connectionId: string, data: {
    name?: string
    endpoint?: string
    models?: { best?: string; balanced?: string; fast?: string }
  }, apiKey?: string) => ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_UPDATE, connectionId, data, apiKey),
  deleteConnection: (connectionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_DELETE, connectionId),
  getActiveConnectionId: () => ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_GET_ACTIVE),
  setActiveConnection: (connectionId: string | null) =>
    ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_SET_ACTIVE, connectionId),
  testConnectionById: (connectionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_TEST, connectionId),
  getConnectionApiKey: (connectionId: string): Promise<string | null> =>
    ipcRenderer.invoke(IPC_CHANNELS.CONNECTIONS_GET_API_KEY, connectionId),
  ...
  getWorkspaceEnv: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.WORKSPACE_ENV_GET, workspaceId),
  saveWorkspaceEnv: (workspaceId: string, vars: { key: string; value: string }[]) =>
    ipcRenderer.invoke(IPC_CHANNELS.WORKSPACE_ENV_SAVE, workspaceId, vars),
  ...
  fsCreateDir: (dirPath: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.FS_MKDIR, dirPath),
  fsRename: (oldPath: string, newPath: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.FS_RENAME, oldPath, newPath),
  fsDelete: (filePath: string, recursive?: boolean) =>
    ipcRenderer.invoke(IPC_CHANNELS.FS_DELETE, filePath, recursive),
  ...
  // USER.md
  readUserMd: () => ipcRenderer.invoke(IPC_CHANNELS.USERMD_READ),
  writeUserMd: (content: string) => ipcRenderer.invoke(IPC_CHANNELS.USERMD_WRITE, content),
  ...
  // Session Info Panel
  getSessionFiles: (sessionId: string, includeHidden?: boolean) => ipcRenderer.invoke(IPC_CHANNELS.GET_SESSION_FILES, sessionId, includeHidden),
  ...
  getDirectoryFiles: (dirPath: string, includeHidden?: boolean) => ipcRenderer.invoke(IPC_CHANNELS.GET_DIRECTORY_FILES, dirPath, includeHidden),
  watchDirectoryFiles: (dirPath: string) => ipcRenderer.invoke(IPC_CHANNELS.WATCH_DIRECTORY_FILES, dirPath),
  unwatchDirectoryFiles: () => ipcRenderer.invoke(IPC_CHANNELS.UNWATCH_DIRECTORY_FILES),
  onDirectoryFilesChanged: (callback: (dirPath: string) => void) => {
  ...
  // Skills catalog — all skills with disabled state
  getSkillsCatalog: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SKILLS_GET_CATALOG, workspaceId),
  toggleSkillEnabled: (workspaceId: string, skillSlug: string, enabled: boolean) =>
    ipcRenderer.invoke(IPC_CHANNELS.SKILLS_TOGGLE_ENABLED, workspaceId, skillSlug, enabled),
  openSkillsDir: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SKILLS_OPEN_SKILLS_DIR, workspaceId),
  pickSkillImportFolder: () =>
    ipcRenderer.invoke(IPC_CHANNELS.SKILLS_IMPORT_PICK_FOLDER),

  // Skills change listener (live updates when skills are added/removed/modified)
  onSkillsChanged: (callback: (skills: import('@opentomo/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@opentomo/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
  ...
  onDisabledSkillsChanged: (callback: (disabledSlugs: string[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, disabledSlugs: string[]) => {
      callback(disabledSlugs)
    }
    ipcRenderer.on(IPC_CHANNELS.SKILLS_DISABLED_CHANGED, handler)
    return () => {
      ipcRenderer.removeListener(IPC_CHANNELS.SKILLS_DISABLED_CHANGED, handler)
    }
  },

  // Commands
  getCommands: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.COMMANDS_GET, workspaceId),
  saveCommand: (workspaceId: string, slug: string, name: string, description: string, content: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.COMMANDS_SAVE, workspaceId, slug, name, description, content),
  deleteCommand: (workspaceId: string, commandSlug: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.COMMANDS_DELETE, workspaceId, commandSlug),
  openCommandInEditor: (workspaceId: string, commandSlug: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.COMMANDS_OPEN_EDITOR, workspaceId, commandSlug),
  openCommandInFinder: (workspaceId: string, commandSlug: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.COMMANDS_OPEN_FINDER, workspaceId, commandSlug),

  // Commands change listener (live updates when commands are added/removed/modified)
  onCommandsChanged: (callback: (commands: import('@opentomo/shared/commands').LoadedCommand[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, commands: import('@opentomo/shared/commands').LoadedCommand[]) => {
      callback(commands)
    }
    ipcRenderer.on(IPC_CHANNELS.COMMANDS_CHANGED, handler)
    return () => {
      ipcRenderer.removeListener(IPC_CHANNELS.COMMANDS_CHANGED, handler)
    }
  },
  ...
  createProject: (workspaceId: string, input: any) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROJECTS_CREATE, workspaceId, input),
  updateProject: (workspaceId: string, projectId: string, input: any) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROJECTS_UPDATE, workspaceId, projectId, input),
  deleteProject: (workspaceId: string, projectId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROJECTS_DELETE, workspaceId, projectId),
  reorderProjects: (workspaceId: string, orderedIds: string[]) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROJECTS_REORDER, workspaceId, orderedIds),
  ...
  smartCategorize: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SMART_CATEGORIZE, workspaceId),
  ...
  saveAppTheme: (colors: Record<string, unknown>) => ipcRenderer.invoke(IPC_CHANNELS.THEME_SAVE_APP, colors),
  createPresetTheme: (name: string, colors: Record<string, unknown>) => ipcRenderer.invoke(IPC_CHANNELS.THEME_CREATE_PRESET, name, colors),
  ...
  onAppThemeChange: (callback: (theme: import('@opentomo/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@opentomo/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
  ...
  checkPrerequisites: () => ipcRenderer.invoke(IPC_CHANNELS.PREREQUISITES_CHECK),
  ...
  onDefaultPermissionsChanged: (callback: () => void) => {
    ...
    return () => ipcRenderer.removeListener(IPC_CHANNELS.DEFAULT_PERMISSIONS_CHANGED, handler)
  },

  // Cost Tracking
  getCostStats: (workspaceId?: string) => ipcRenderer.invoke(IPC_CHANNELS.COST_GET_STATS, workspaceId),

  // Menu actions (for unified OpenTomo menu)
  ...
  terminalCreate: (cwd: string, cols: number, rows: number) =>
    ipcRenderer.invoke(IPC_CHANNELS.TERMINAL_CREATE, cwd, cols, rows),
  terminalWrite: (ptyId: string, data: string) =>
    ipcRenderer.send(IPC_CHANNELS.TERMINAL_WRITE, ptyId, data),
  terminalResize: (ptyId: string, cols: number, rows: number) =>
    ipcRenderer.invoke(IPC_CHANNELS.TERMINAL_RESIZE, ptyId, cols, rows),
  terminalKill: (ptyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.TERMINAL_KILL, ptyId),
  onTerminalData: (callback: (ptyId: string, data: string) => void) => {
    const handler = (_e: Electron.IpcRendererEvent, ptyId: string, data: string) => callback(ptyId, data)
    ipcRenderer.on(IPC_CHANNELS.TERMINAL_DATA, handler)
    return () => ipcRenderer.removeListener(IPC_CHANNELS.TERMINAL_DATA, handler)
  },
  onTerminalExit: (callback: (ptyId: string, exitCode: number) => void) => {
    const handler = (_e: Electron.IpcRendererEvent, ptyId: string, code: number) => callback(ptyId, code)
