...
import type { Session, Workspace, SessionEvent, Message, FileAttachment, StoredAttachment, PermissionRequest, CredentialRequest, CredentialResponse, SetupNeeds, NewChatActionParams, ContentBadge } from '../shared/types'
...
import { TooltipProvider } from '@opentomo/ui'
...
export default function App() {
  ...
  // Current selected provider (anthropic, custom)
  ...
  // When a custom connection is active, its full model tier definitions (best, balanced, fast).
  // Populated by refreshCustomModel after any provider switch.
  const [customConnectionModels, setCustomConnectionModels] = useState<{ best?: string; balanced?: string; fast?: string } | null>(null)
  // Current auth type - used to track active authentication method
  ...
  // Guard: prevents the initialization effect from sending the user back to onboarding
  // after the app has already reached 'ready' state (e.g. after a provider switch that
  // changes currentAuthType and re-triggers the effect via its dependency array).
  const appIsReadyRef = useRef(false)
  // Unified session options - all session-scoped options in one place
  ...
  const refreshCustomModel = useCallback(async () => {
    const [billing, storedProvider, connections, activeId, storedModel] = await Promise.all([
    ...
    setCurrentAuthType(billing.authType)
    if (storedProvider) {
      setCurrentProvider(storedProvider)
    }
    // Expose model tiers for the model picker in FreeFormInput
    const activeConn = (connections as Array<{ id: string; models: { best?: string; balanced?: string; fast?: string } }>)
      ?.find(c => c.id === activeId)
    if (activeConn) {
      setCustomConnectionModels(activeConn.models ?? null)
    } else if (billing.authType === 'oauth_token' && billing.oauthTierModels) {
      // Claude Subscription: use dynamically fetched tier models from Anthropic API
      setCustomConnectionModels(billing.oauthTierModels)
    } else {
      setCustomConnectionModels(null)
    }
    // Sync currentModel from config (handles provider switch)
  ...
  // Check auth state and get window's workspace ID.
  useEffect(() => {
    const initialize = async () => {
      // Skip re-initialization if the app has already reached 'ready' state.
      // refreshCustomModel() changes currentAuthType which re-triggers this effect,
      // but there is no need to re-check setup needs after the app is ready.
      if (appIsReadyRef.current) return

      try {
        // Fetch billing info FIRST — needed to set currentAuthType for other effects.
        // On the first render currentAuthType is null; reading authType here ensures
        // the auth type is known before any further checks.
        ...
        // Only update if we received a real value — don't clobber a valid workspaceId with null
        // when this effect re-runs due to currentAuthType changing.
        ...
        if (needs.needsWorkspace) {
          // Workspace not yet created → run onboarding
          setAppState('onboarding')
        } else {
          // Workspace exists → go ready; API connection configured in Settings
          appIsReadyRef.current = true
          setAppState('ready')
        }
      } catch (error) {
  ...
  // Watchdog: prevent indefinite loading. If still 'loading' after 10s, go to onboarding.
  useEffect(() => {
    if (appState !== 'loading') return
    const timer = setTimeout(() => {
      setAppState(prev => prev === 'loading' ? 'onboarding' : prev)
      // Minimal setupNeeds to ensure onboarding knows configuration is incomplete
      setSetupNeeds({ needsBillingConfig: true, needsCredentials: true, needsWorkspace: true, isFullyConfigured: false })
    }, 10_000)
  ...
  useEffect(() => {
    ...
    window.electronAPI.getSessions().then((loadedSessions) => {
      ...
      console.error('[App] getSessions failed:', err)
      // Still allow app to proceed; show UI without sessions
      setSessionsLoaded(true)
    })
    // Load stored provider preference
    window.electronAPI.getProvider().then((storedProvider) => {
      if (storedProvider) {
        setCurrentProvider(storedProvider)
      }
    })
    ...
    // Load custom model, auth type, provider, and active connection tier models on startup
    refreshCustomModel()
  ...
  // Defense-in-depth: if getSessions() was called before main finished loadSessionsFromDisk()
  // (race condition fallback), re-fetch when main signals sessions are ready.
  useEffect(() => {
    const unsubscribe = window.electronAPI.onSessionsInitialized(() => {
      window.electronAPI.getSessions().then((loadedSessions) => {
        if (loadedSessions.length > 0) {
          initializeSessions(loadedSessions)
        }
      }).catch(() => { /* ignore — main process will show empty list */ })
    })
    return unsubscribe
  }, [initializeSessions])
  ...
  useEffect(() => {
    ...
    const handoffEventTypes = new Set(['complete', 'error', 'interrupted', 'typed_error', 'name_changed', 'labels_changed', 'project_changed', 'title_generated', 'async_operation'])
    ...
    const cleanup = window.electronAPI.onSessionEvent((event: SessionEvent) => {
      ...
      // Latency measurement: log time from send to first event / first text_delta
      if (typeof window !== 'undefined' && (window as any).__lastSendSessionId === sessionId) {
        const sendTs = (window as any).__lastSendTimestamp as number | undefined
        if (sendTs) {
          const elapsed = performance.now() - sendTs
          if (!(window as any).__firstEventLogged) {
            console.debug(`[PERF] renderer.first_event: ${elapsed.toFixed(1)}ms`)
            ;(window as any).__firstEventLogged = true
          }
          if (event.type === 'text_delta' && !(window as any).__firstTextDeltaLogged) {
            console.debug(`[PERF] renderer.first_text_delta: ${elapsed.toFixed(1)}ms`)
            ;(window as any).__firstTextDeltaLogged = true
          }
          if (event.type === 'complete') {
            // Reset for next message
            ;(window as any).__firstEventLogged = false
            ;(window as any).__firstTextDeltaLogged = false
          }
        }
      }
      ...
      if (event.type === 'info' && event.statusType === 'compaction_complete') {
        window.dispatchEvent(new CustomEvent('ss:compaction-complete', {
          detail: { sessionId }
        }))
      }
      ...
      if (isStreaming || isHandoff) {
        ...
        // Skip atom update if session wasn't loaded — writing a fabricated empty session
        // would corrupt the atom with isProcessing:true and missing data
        if (atomSession) {
        ...
        if (isHandoff) {
          ...
          if (atomSession) {
            // Session was loaded — processed result has full data, safe to derive meta
            newMetaMap.set(sessionId, extractSessionMeta(updatedSession))
          } else {
            // Session was NOT loaded — updatedSession is based on a fabricated empty session.
            // Don't replace the existing meta (would corrupt name, dates, etc.).
            // Instead, patch only the event-specific field on the existing meta.
            const existing = metaMap.get(sessionId)
            if (existing) {
              const patched = { ...existing }
              switch (event.type) {
                case 'project_changed': patched.projectId = (event as SessionEvent & { projectId: string | null }).projectId; break
                case 'name_changed': patched.name = (event as SessionEvent & { name?: string }).name; break
                case 'title_generated': patched.name = (event as SessionEvent & { title: string }).title; break
                case 'async_operation': {
                  const isOngoing = (event as SessionEvent & { isOngoing: boolean }).isOngoing
                  patched.isAsyncOperationOngoing = isOngoing
                  patched.isRegeneratingTitle = isOngoing
                  break
                }
              }
              newMetaMap.set(sessionId, patched)
            }
          }
          ...
          if (event.type === 'complete' && atomSession && !updatedSession.hidden) {
  ...
  const handleSendMessage = useCallback(async (sessionId: string, message: string, attachments?: FileAttachment[], skillSlugs?: string[], externalBadges?: ContentBadge[]) => {
    try {
      ...
      // Step 3: Check if designAgent is enabled for this session
      const isDesignAgent = sessionOptions.get(sessionId)?.designAgentEnabled ?? false
      ...
      // Record send timestamp for latency measurement (visible in DevTools Performance tab)
      const sendTimestamp = performance.now()
      if (typeof window !== 'undefined') {
        ;(window as any).__lastSendTimestamp = sendTimestamp
        ;(window as any).__lastSendSessionId = sessionId
      }
      ...
      await window.electronAPI.sendMessage(sessionId, message, processedAttachments, storedAttachments, {
        skillSlugs,
        badges: badges.length > 0 ? badges : undefined,
        designAgentEnabled: isDesignAgent || undefined,
      })
    } catch (error) {
  ...
  // Execute logout - clears credentials only, preserves workspaces/sessions
  const handleLogout = useCallback(async () => {
    try {
      // Clear credentials and auth env vars in the main process
      await window.electronAPI.logout()

      // Workspace is preserved — stay ready so user can re-configure API via Settings
      appIsReadyRef.current = true
      setAppState('ready')
    } catch (error) {
  ...
  const handleSelectWorkspace = useCallback(async (workspaceId: string, openInNewWindow = false) => {
    ...
    if (openInNewWindow) {
      ...
      // 4. Clear ALL workspace-scoped state BEFORE navigating.
      // navigate() fires a synchronous DOM event that triggers applyNavigationState,
      // which auto-selects the first session via getFirstSessionId(). If session atoms
      // still contain old workspace data, a session from the wrong workspace gets selected.
      // Clearing atoms first ensures getFirstSessionId() finds nothing to auto-select.
      ...
      // 5. Navigate to allChats view without a specific session selected
      // This ensures the UI is in a clean state for the new workspace.
      // Must happen AFTER atoms are cleared (step 4) so auto-selection doesn't
      // pick a session from the previous workspace.
  ...
  // Platform actions for @opentomo/ui components (overlays, etc.)
  ...
  if (appState === 'loading') {
    return (
      <SplashScreen
        isExiting={false}
        updateInfo={updateChecker.isDownloading && updateChecker.updateInfo?.latestVersion ? {
    ...
    return (
      <ModalProvider>
        ...
        <OnboardingWizard
          ...
          onRecheckPrerequisites={onboarding.handleRecheckPrerequisites}
 ...
 * - svg → SVGPreviewOverlay (text-based, with zoom/pan and code view)
