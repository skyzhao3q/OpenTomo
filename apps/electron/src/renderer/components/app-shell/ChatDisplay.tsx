...
import { CollapsibleMarkdownProvider, StreamingMarkdown, MarkdownMediaProvider, type RenderMode, type MarkdownMediaActions } from "@/components/markdown"
...
import { TurnCard, UserMessageBubble, groupMessagesByTurn, formatTurnAsMarkdown, formatActivityAsMarkdown, type Turn, type AssistantTurn, type UserTurn, type SystemTurn, type AuthRequestTurn } from "@opentomo/ui"
...
import { agentNameAtom, userNameAtom } from "@/atoms/agent-name"
...
interface ChatDisplayProps {
  ...
  /** Enable Design Agent mode for image generation */
...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  // Media actions for Markdown rendering (image generation removed)
  const mediaActions = React.useMemo<MarkdownMediaActions>(() => ({
    sessionId: session?.id,
    generate: async () => ({ success: false, error: 'Image generation not supported' }),
    cancelGenerate: async () => {},
    showInFolder: (path) => window.electronAPI.showInFolder(path),
    openFile: async (path) => { onOpenFile(path) },
  }), [session?.id, onOpenFile])

  // Register widget globals so iframes can send messages to the agent and open URLs
  React.useEffect(() => {
    window.__widgetSendMessage = (message: string) => {
      if (message?.trim()) {
        handleSubmit(message.trim())
      }
    }
    window.__widgetOpenUrl = (url: string) => {
      onOpenUrl(url)
    }
    return () => {
      delete window.__widgetSendMessage
      delete window.__widgetOpenUrl
    }
  // handleSubmit and onOpenUrl are stable references (defined inside component)
  ...
  // These settings are stored in ~/.opentomo/preferences.json (not localStorage)
  ...
  // Uses the shared extractOverlayData parser from @opentomo/ui
  ...
  return (
    <MarkdownMediaProvider value={mediaActions}>
      ...
      {session ? (
        <div className="flex flex-1 flex-col min-h-0 min-w-0 relative">
            ...
            >
              ...
              <div className={cn(CHAT_LAYOUT.maxWidth, "mx-auto", CHAT_LAYOUT.containerPadding, CHAT_LAYOUT.messageSpacing, "min-w-0")}>
                ...
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  />
                  {/* Empty session welcome - shown when no messages yet (regular mode only) */}
                  {!compactMode && turns.length === 0 && (
                    <div className="flex flex-col items-center justify-center gap-5 min-h-[50vh] py-10 select-none pointer-events-none">
                      <AgentAvatar className="h-40 w-40" />
                      <div className="text-center space-y-2">
                        <p className="text-base font-medium text-foreground/80">
                          {userName
                            ? t('chat.emptyState.greeting', { name: userName })
                            : t('chat.emptyState.greetingNoName')}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {t('chat.emptyState.subtitle', { agentName })}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {turns.map((turn, index) => {
                    ...
                    return (
                      ...
                      <TurnCard
                        ...
                        compactMode={compactMode}
                        onAcceptPlan={() => {
                          window.dispatchEvent(new CustomEvent('ss:approve-plan', {
                        ...
                        onAcceptPlanWithCompact={() => {
                          ...
                          window.dispatchEvent(new CustomEvent('ss:approve-plan-with-compact', {
                        ...
                        agentIcon={<AgentAvatar className="h-10 w-10" />}
                      />
                      </div>
                    )
                  })}
          ...
          <div className={cn(
            ...
            {!compactMode && (
            <ActiveOptionBadges
              designAgentEnabled={designAgentEnabled}
              onDesignAgentChange={onDesignAgentChange}
              permissionMode={permissionMode}
    ...
    return (
      <div className="flex justify-start group">
        <div className="relative max-w-[90%] bg-background shadow-minimal rounded-[8px] pl-6 pr-4 py-3 break-words min-w-0 select-text">
          {/* Always use CollapsibleMarkdownProvider + StreamingMarkdown so that
              WidgetRenderer instances keep a stable React key (w-N) across the
              streaming→non-streaming transition. Switching to a bare <Markdown>
              would unmount every iframe and reload CDN scripts. */}
