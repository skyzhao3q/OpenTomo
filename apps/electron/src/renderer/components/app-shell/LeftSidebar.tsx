...
import { Tooltip, TooltipTrigger, TooltipContent } from '@opentomo/ui'
...
interface LeftSidebarProps {
  ...
  isNested?: boolean
  /** Icon-only mode: hide text labels and show tooltips instead */
  isIconOnly?: boolean
}
 ...
 */
export function LeftSidebar({ links, isCollapsed, getItemProps, focusedItemId, isNested, isIconOnly }: LeftSidebarProps) {
    ...
    <div className={cn("flex flex-col select-none", !isNested && "py-1")}>
      ...
      >
        ...
        {links.map((item) => {
          ...
          // Inner content (with optional context menu)
          const innerContent = (
            <div className="group/section">
              ...
              {/* Expandable subitems — suppressed in icon-only mode */}
              {!isIconOnly && link.expandable && link.items && (
          ...
          // In icon-only mode, wrap with tooltip showing the title
...
interface SidebarButtonProps {
  ...
  isOverlay?: boolean
  /** Icon-only mode: hide text, center the icon */
  isIconOnly?: boolean
}
...
const SidebarButton = React.forwardRef<HTMLButtonElement, SidebarButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ link, itemProps, isOverlay, isIconOnly, className: extraClassName, ...radixProps }, forwardedRef) => {
    return (
      <button
        {...(isOverlay ? {} : (() => {
        ...
        className={cn(
          "group flex w-full items-center rounded-[6px] text-[13px] select-none outline-none",
      ...
      >
        ...
        <span className="relative h-3.5 w-3.5 shrink-0 flex items-center justify-center">
          {link.expandable && !isOverlay && !isIconOnly ? (
            <>
        ...
        {/* Text labels and badges — hidden in icon-only mode */}
        {!isIconOnly && (
          <>
            {link.title}
