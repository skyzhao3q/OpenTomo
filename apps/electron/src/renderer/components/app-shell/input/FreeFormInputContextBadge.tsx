...
export interface FreeFormInputContextBadgeProps {
  ...
  showChevron?: boolean
  /** When true, never shows label text (icon only), but keeps selection styling */
  iconOnly?: boolean
  /** Click handler */
  onClick?: () => void
...
export const FreeFormInputContextBadge = React.forwardRef<HTMLButtonElement, FreeFormInputContextBadgeProps>(
  function FreeFormInputContextBadge(
    ...
    // Show label in expanded state OR in collapsed state with selection (unless iconOnly)
    const showLabel = !iconOnly && (isExpanded || hasSelection)
    ...
    const button = (
      ...
      >
        ...
        {/* Optional chevron - only in expanded state and not iconOnly */}
        {!iconOnly && isExpanded && showChevron && (
