...
import { usePlatform } from '@opentomo/ui'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        'The user is editing app-level default permissions (~/.opentomo/permissions/default.json). ' +
  ...
  // Preferences editing context (legacy, kept for compatibility)
  'preferences-notes': (location) => ({
    context: {
      label: 'Preferences Notes',
      filePath: location,
      context:
        'The user is editing the notes field in their preferences (~/.opentomo/preferences.json). ' +
  ...
  // User notes editing context (USER.md)
  'user-notes': (location) => ({
    context: {
      label: 'User Notes',
      filePath: location, // location is the full path to USER.md
      context:
        'The user is editing the notes section of their USER.md file (~/.opentomo/USER.md). ' +
        'This is a markdown file with YAML front matter (name, timezone, language, city, country) and a body section for free-form notes. ' +
        'Only modify the body section (after the closing ---) unless explicitly asked otherwise. ' +
        'The notes provide context about the user to the AI agent. ' +
        'Confirm clearly when done.',
    },
    example: 'Add my coding style preferences',
    model: 'haiku',
    systemPromptPreset: 'mini',
    inlineExecution: true,
  }),

  // Soul/personality editing context (SOUL.md)
  'soul-personality': (location) => ({
    context: {
      label: 'Agent Personality',
      filePath: location, // location is the full path to SOUL.md
      context:
        'The user is editing the personality section of their SOUL.md file (~/.opentomo/SOUL.md). ' +
        'This is a markdown file with YAML front matter (agentName) and a body section for personality definition. ' +
        'Only modify the body section (after the closing ---) unless explicitly asked otherwise. ' +
        'The body defines the agent\'s tone, style, and behavioral guidelines. ' +
        'Confirm clearly when done.',
    },
    example: 'Make the agent more concise and technical',
    model: 'haiku',
    systemPromptPreset: 'mini',
    inlineExecution: true,
  }),

  // Add new skill contexts - use overridePlaceholder for inspiring, contextual prompts
  'add-skill': (location) => ({
    context: {
      label: 'Add Skill',
      ...
      context:
        ...
        'Follow the patterns in ~/.opentomo/docs/skills.md. ' +
  ...
  'add-command': (location) => ({
    context: {
      label: 'Add Command',
      filePath: `${location}/commands/`, // location is the workspace root path
      context:
        'The user wants to add a new command to their workspace. ' +
        'Commands are pre-defined prompts stored as .md files with YAML frontmatter (name, description) and prompt content as the body. ' +
        'Ask clarifying questions if needed: What prompt should this command contain? ' +
        'Create the .md file in the workspace commands directory. ' +
        'The filename (without extension) becomes the command slug.',
    },
    example: 'Summarize the current codebase',
    overridePlaceholder: 'What prompt should this command contain?',
  }),
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.opentomo/docs/labels.md for full format reference. ' +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is tool-icons.json in ~/.opentomo/tool-icons/. Icon image files live in the same directory. ' +
        ...
        'Read ~/.opentomo/docs/tool-icons.md for full format reference. ' +
...
export function EditPopover({
  ...
  // overridePlaceholder allows contexts like add-skill to say "add" instead of "change"
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = `opentomo://action/new-chat?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`
