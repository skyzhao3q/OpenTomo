...
import { PERMISSION_MODE_CONFIG, PERMISSION_MODE_ORDER, type PermissionMode } from '@opentomo/shared/agent/modes'
// ============================================================================
// Types
// ============================================================================
...
// i18n key mapping for mode names (used by components to translate mode labels)
export const MODE_I18N_KEYS: Record<PermissionMode, { label: string; short: string; desc: string }> = {
  'safe':      { label: 'modes.planModeLabel',          short: 'modes.planModeLabel',          desc: 'modes.planModeDesc' },
  'ask':       { label: 'modes.askBeforeEditsLabel',    short: 'modes.askBeforeEditsLabel',    desc: 'modes.askBeforeEditsDesc' },
  'allow-all': { label: 'modes.editAutomaticallyLabel', short: 'modes.editAutomaticallyLabel', desc: 'modes.editAutomaticallyDesc' },
}
...
/** Create localized command groups using i18n translation function */
export function createLocalizedCommandGroups(t: (key: string, defaultValue: string) => string): CommandGroup[] {
  const modeCommands: SlashCommand[] = PERMISSION_MODE_ORDER.map(mode => {
    const config = PERMISSION_MODE_CONFIG[mode]
    const keys = MODE_I18N_KEYS[mode]
    return {
      id: mode as SlashCommandId,
      label: t(keys.label, config.displayName),
      description: t(keys.desc, config.description),
      icon: <PermissionModeIcon mode={mode} className={MENU_ICON_SIZE} />,
    }
  })
  return [
  ...
  return (
    ...
    >
      <div ref={listRef} className={MENU_LIST_STYLE}>
        {flatItems.length === 0 ? (
          <div className="px-3 py-3 text-[13px] text-muted-foreground select-none">
...
export function useInlineSlashCommand({
  ...
  // No sections (prompt commands removed)
  ...
  const handleInputChange = React.useCallback((value: string, cursorPosition: number) => {
    ...
    const slashMatch = textBeforeCursor.match(/(?:^|\s)\/([\p{L}\p{N}_-]*)$/u)
