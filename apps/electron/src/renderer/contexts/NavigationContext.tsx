...
export { isChatsNavigation, isSettingsNavigation, isSkillsNavigation }
...
export function NavigationProvider({
  ...
  // Jotai store for reading latest atom values directly (avoids stale closure issues)
  const store = useStore()

  // Helper: Filter sessions by ChatFilter
  // Always excludes hidden sessions and sessions from other workspaces
  const filterSessionsByFilter = useCallback(
    (filter: ChatFilter): SessionMeta[] => {
      // Filter out hidden sessions and sessions from other workspaces
      const visibleSessions = sessionMetas.filter(s =>
        !s.hidden && (!workspaceId || s.workspaceId === workspaceId)
      )
  ...
  // Reads directly from the Jotai store to get the freshest data.
  // During workspace switch, React state (useAtomValue) is stale until re-render,
  // but store.get() reflects synchronous atom updates (e.g., clearing atoms in
  // handleSelectWorkspace). This prevents auto-selecting sessions from the old workspace.
  const getFirstSessionId = useCallback(
    (filter: ChatFilter): string | null => {
      const latestMetas = Array.from(store.get(sessionMetaMapAtom).values())
      const visibleSessions = latestMetas.filter(s =>
        !s.hidden && (!workspaceId || s.workspaceId === workspaceId)
      )
  ...
  // Helper: Get first skill slug (excluding builtin skills)
  const getFirstSkillSlug = useCallback(
    (): string | null => {
      // Filter out builtin skills - they're hidden from UI
      const customSkills = skills.filter(s => s.source !== 'builtin')
      return customSkills[0]?.slug ?? null
    },
    [skills]
  )
  ...
  const handleActionNavigation = useCallback(
    async (parsed: ParsedRoute) => {
      if (!workspaceId) {
        console.warn('[Navigation] handleActionNavigation: workspaceId is null, cannot process action:', parsed.name)
        return
      }
      ...
      switch (parsed.name) {
        case 'new-chat': {
          try {
            ...
            const filter: import('../../shared/types').ChatFilter = { kind: 'allChats' }
            ...
            console.error('[Navigation] Failed to create new chat session:', error)
          }
          break
        }
        ...
        case 'checkout-success':
          // Navigate to providers settings page and trigger subscription refresh
          setNavigationState({ navigator: 'settings', subpage: 'providers' })
    ...
    [workspaceId, onCreateSession, onInputChange, setSession, setNavigationState]
  )
  ...
  const applyNavigationState = useCallback(
    ...
    [getFirstSessionId, getFirstSkillSlug, setSession]
  )
