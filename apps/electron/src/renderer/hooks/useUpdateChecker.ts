...
export function useUpdateChecker(): UseUpdateCheckerResult {
  ...
  // Callback for showing release notes (set by Settings page or other consumers)
  const [onShowReleaseNotes, setOnShowReleaseNotes] = useState<(() => void) | null>(null)

  // Show toast notification when update is ready
  const showUpdateToast = useCallback((version: string, onInstall: () => void, onWhatsNew?: () => void) => {
    ...
    toast.info(`Update v${version} ready`, {
      id: UPDATE_TOAST_ID,
      description: 'New version available. Restart to update.',
      duration: 15000, // 15 seconds (increased from 10)
      action: {
        label: 'Restart',
        onClick: onInstall,
      },
      // Add "What's New" as cancel button if callback provided
      cancel: onWhatsNew ? {
        label: "What's New",
        onClick: onWhatsNew,
      } : undefined,
  ...
  useEffect(() => {
    const checkAndNotify = async (info: UpdateInfo) => {
      ...
      // Show toast for ready update (no "What's New" button in hook - added by Settings page)
  ...
  const checkForUpdates = useCallback(async () => {
    try {
      ...
      // The error message from main process is already user-friendly
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'

      toast.error('Failed to check for updates', {
        description: errorMessage,
        duration: 8000, // Longer duration for network error messages
      })
    }
  }, [showUpdateToast, installUpdate])

  const showReleaseNotes = useCallback(() => {
    if (onShowReleaseNotes) {
      onShowReleaseNotes()
    }
  }, [onShowReleaseNotes])
