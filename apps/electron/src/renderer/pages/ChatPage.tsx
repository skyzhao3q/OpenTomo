...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  // Atom updater for optimistic session model updates
  ...
  // Session model change handler - optimistically updates atom then persists per-session model
  const handleModelChange = React.useCallback((model: string) => {
    // Optimistic update so the dropdown label reflects the new model immediately
    updateSessionDirect(sessionId, (prev) => prev ? { ...prev, model } : prev)
    if (activeWorkspaceId) {
      window.electronAPI.setSessionModel(sessionId, activeWorkspaceId, model)
    }
  }, [sessionId, activeWorkspaceId, updateSessionDirect])
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `opentomo://${route}${separator}window=focused`
    try {
  ...
  if (!session) {
      ...
      return (
        <>
          <div className="h-full flex flex-col">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <ChatDisplay
                ...
                designAgentEnabled={sessionOpts.designAgentEnabled}
                onDesignAgentChange={(enabled) => setOption('designAgentEnabled', enabled)}
          ...
          <RenameDialog
            ...
            title={t('chatPage.renameChat')}
            value={renameName}
            onValueChange={setRenameName}
            onSubmit={handleRenameSubmit}
            placeholder={t('chatPage.enterChatName')}
          />
        </>
      )
    }
    ...
    return (
      <div className="h-full flex flex-col">
        <PanelHeader  title={t('chatPage.chat')} rightSidebarButton={rightSidebarButton} />
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          <AlertCircle className="h-10 w-10" />
          <p className="text-sm">{t('chatPage.sessionNotExist')}</p>
        </div>
      </div>
    )
  }
