...
const COMPOUND_ROUTE_PREFIXES = [
  'allChats', 'view', 'sources', 'skills', 'settings'
]
...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  if (first === 'settings') {
    ...
    const validSubpages: SettingsSubpage[] = ['app', 'appearance', 'input', 'workspace', 'permissions', 'shortcuts', 'skills', 'providers']
  ...
  if (first === 'sources') {
    ...
    // Source selection: sources/source/{sourceSlug}
...
function convertCompoundToViewRoute(compound: ParsedCompoundRoute): ParsedRoute {
  ...
  if (compound.chatFilter) {
    ...
    return {
      type: 'view',
      name: filter.kind,
      id: filter.kind === 'view' ? filter.viewId : undefined,
      params: {},
    }
  }
  ...
  switch (parsed.name) {
    ...
    case 'chat':
      if (parsed.id) {
        ...
        if (filterKind === 'view' && parsed.params.viewId) {
          filter = { kind: 'view', viewId: parsed.params.viewId }
        } else {
          filter = { kind: filterKind as 'allChats' }
        }
        return {
          navigator: 'chats',
          filter,
