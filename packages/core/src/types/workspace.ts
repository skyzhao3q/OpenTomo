 ...
 */
export type McpAuthType = 'workspace_bearer' | 'public';

export interface Workspace {
  id: string;
  name: string;            // Read from workspace folder config (not stored in global config)
  rootPath: string;        // Absolute path to workspace folder (e.g., ~/Projects/my-app/opentomo)
  createdAt: number;
...
export interface StoredConfig {
  provider?: string;               // NEW: Selected AI provider ('opentomo', 'anthropic', 'custom')
  authType?: AuthType;
