...
This guide explains how to create and configure skills in opentomo.
...
opentomo uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in opentomo
...
1. **Workspace skill checked first** - If `~/.opentomo/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.opentomo/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.opentomo/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.opentomo/workspaces/{ws}/skills/commit/SKILL.md`
