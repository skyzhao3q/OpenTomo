...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in opentomo.
...
**Before doing anything else**, search for a specialized guide using the opentomo-docs MCP:

```
mcp__opentomo-docs__SearchOpenTomo({ query: "{service} source setup" })
```
...
- "To find issues in the Backend API project: ..."
...
Agent: [FIRST: Searches for Linear guide]
       mcp__opentomo-docs__SearchOpenTomo({ query: "linear source setup" })
...
User: Issue tracking for my iOS team, project called "Backend API"
...
Agent: [Creates config.json with:
        ...
        - tagline: "Issue tracking for the Backend API team"
        - OAuth authentication]
Agent: [Writes guide.md mentioning "Backend API" project and iOS team context]
...
Agent: Linear is now set up! I've configured it to focus on the "Backend API" project.
You can use it to:
- Search and view issues in Backend API
...
- `~/.opentomo/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.opentomo/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.opentomo/workspaces/{ws}/sources/my-source
   ```
