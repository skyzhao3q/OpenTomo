...
This guide explains how to customize the visual theme of opentomo.

## Overview

opentomo uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.opentomo/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.opentomo/theme.json` - Override specific colors (app-level)
...
```
~/.opentomo/workspaces/{id}/config.json
```
...
Create `~/.opentomo/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.opentomo/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.opentomo/themes/{name}.json`
...
1. Create `~/.opentomo/theme.json` for overrides or `~/.opentomo/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.opentomo/theme.json` for overrides, `~/.opentomo/themes/` for presets)
