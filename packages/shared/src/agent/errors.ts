...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  invalid_api_key: {
    ...
    message: 'Your API credentials were rejected. They may be invalid or expired.',
    actions: [
  ...
  mcp_unreachable: {
    ...
    message: 'Cannot connect to the OpenTomo MCP server. Check your network connection.',
    actions: [
      { key: 'r', label: 'Retry', action: 'retry' },
    ],
    canRetry: true,
  ...
  billing_error: {
    ...
    message: 'Your account has a billing issue. Check your account status.',
    actions: [
      { key: 's', label: 'Update credentials', command: '/settings', action: 'settings' },
    ],
    canRetry: false,
  ...
  model_no_tool_support: {
    ...
    message: `The selected model does not support tool/function calling, which is required for ${getAgentName()}. Please choose a model with tool support (e.g., Claude, GPT-4, Gemini).`,
    actions: [
...
export function parseError(error: unknown): AgentError {
  ...
  // Tool support errors must be checked BEFORE model errors since tool errors often contain "model"
    ...
    // Azure-format error returned when a custom provider (e.g. OpenTomo proxy) receives
    // a model name that doesn't match any deployment (e.g. "qwen3:8b" on Azure OpenAI)
    (lowerMessage.includes('deployment') && lowerMessage.includes('does not exist')) ||
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for ${getAgentName()}. Please choose a different model with tool support in Settings.`,
