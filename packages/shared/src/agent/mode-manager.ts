...
class ModeManager {
  ...
  setPermissionMode(sessionId: string, mode: PermissionMode): void {
    ...
    // Notify callbacks (for OpenTomoAgent internal sync)
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    // Always allow preferences, documentation, and memory tools
    if (toolName.startsWith('mcp__preferences__') || toolName.startsWith('mcp__opentomo-docs__')) {
      return { allowed: true };
    }
    // Memory tools are always allowed (user's own memory data, not code files)
    if (toolName.startsWith('mcp__memory__')) {
      return { allowed: true };
    }
    // Widget guidelines tool is always allowed (read-only, no side effects)
    if (toolName.startsWith('mcp__opentomo-widget__')) {
      return { allowed: true };
    }
