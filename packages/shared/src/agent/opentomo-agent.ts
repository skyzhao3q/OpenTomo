...
// Plan types are used by UI components; not needed in opentomo-agent.ts since Safe Mode is user-controlled
import { parseError, type AgentError } from './errors.ts';
import { runErrorDiagnostics } from './diagnostics.ts';
import { loadStoredConfig, getAnthropicBaseUrl, getAuthType, resolveModelId, getDefaultChatMode, type Workspace } from '../config/storage.ts';
import { isLocalMcpEnabled, loadWorkspaceConfig } from '../workspaces/storage.ts';
import { readWorkspaceEnvFile } from '../workspaces/env-storage.ts';
import { loadPlanFromPath, type SessionConfig as Session } from '../sessions/storage.ts';
import { DEFAULT_MODEL, isClaudeModel } from '../config/models.ts';
import { getAgentName } from '../prompts/soul.ts';
import { formatUserContextForPrompt, migrateFromPreferencesJson } from '../prompts/user-context.ts';
import type { FileAttachment } from '../utils/files.ts';
import { debug } from '../utils/debug.ts';
import { perf, type PerfSpan } from '../utils/perf.ts';
import {
  getSessionPlansDir,
  getLastPlanFilePath,
  clearPlanFileState,
...
import { createMemoryTools } from '../memory/index.ts';
import { getWidgetGuidelinesServer } from '../widget/widget-guidelines.ts';
import {
  getPermissionMode,
  setPermissionMode,
  cyclePermissionMode,
...
import { buildAgentEnvironment, getGlobalRegistry } from '../runtimes/index.ts';
...
// Documentation is served via local files at ~/.opentomo/docs/

// Import and re-export AgentEvent from core (single source of truth)
import type { AgentEvent } from '@opentomo/core/types';
...
// Minimal LoadedSource type (sources/types.ts deleted; kept for backward compatibility with UI consumers)
...
/**
 * Resolve allowed SDK beta headers safely.
 * - Do not send betas when routed through a proxy or custom base URL
 * - Allow opt-in context-1m beta via env flag only (conservative default: none)
 */
function resolveAllowedBetas(isProxy: boolean, activeBaseUrl: string | null | undefined, model: string): string[] {
  // Proxies and third-party providers may reject Anthropic beta headers
  if (isProxy) return [];
  if (activeBaseUrl) return [];

  // Optional opt-in for 1M context beta. Disabled by default to avoid warnings.
  if (process.env.CLAUDE_ENABLE_1M_BETA === '1') {
...
export interface OpenTomoAgentConfig {
  workspace: Workspace;
  ...
  /** Active auth provider — used for provider-specific error messages and retry logic */
  authType?: import('../config/storage.ts').AuthType;
  ...
  /** Active connector IDs for system prompt guidelines injection */
  connectorIds?: string[];
  /** Enable generative UI widgets (show-widget fences). Defaults to true. */
  generativeUiEnabled?: boolean;
  /** Custom skill slugs that are disabled for this workspace. Calls to these skills will be blocked. */
...
// Preferences MCP server removed — preferences.json is replaced by USER.md/SOUL.md.
// User context is managed via memory tools (memory_write to /global/user-context.md).

// Cached MCP server for memory (keyed by workspaceRootPath)
const memoryServerCache = new Map<string, ReturnType<typeof createSdkMcpServer>>();

// Memory MCP server - episode memory tools (memory_save, memory_search, memory_read)
function getMemoryServer(workspaceRootPath: string): ReturnType<typeof createSdkMcpServer> {
  let cached = memoryServerCache.get(workspaceRootPath);
  if (!cached) {
    cached = createMemoryTools(workspaceRootPath);
    memoryServerCache.set(workspaceRootPath, cached);
  }
  return cached;
}
...
export class OpenTomoAgent {
  private config: OpenTomoAgentConfig;
  ...
  // Temporary clarifications (not yet saved to document)
  ...
  private pinnedContextPrompt: string | null = null;
  // Cached system prompt (avoids re-reading files and re-running globSync on every message)
  private cachedSystemPrompt: string | null = null;
  // Track if preference drift notification has been shown this session
  private contextDriftNotified: boolean = false;
  ...
  constructor(config: OpenTomoAgentConfig) {
    ...
    debug(`[OpenTomoAgent] Using model: ${model}`);

    // Migrate preferences.json → USER.md / SOUL.md (runs once if USER.md doesn't exist)
    migrateFromPreferencesJson();
    ...
    // Get initial mode: from session config, or from user's configured default
    const initialMode: PermissionMode = config.session?.permissionMode ?? getDefaultChatMode();
    ...
    registerSessionScopedToolCallbacks(sessionId, {
      onPlanSubmitted: (planPath) => {
        this.onDebug?.(`[OpenTomoAgent] onPlanSubmitted received: ${planPath}`);
        this.onPlanSubmitted?.(planPath);
      },
      onAuthRequest: (request) => {
        this.onDebug?.(`[OpenTomoAgent] onAuthRequest received: ${request.sourceSlug} (type: ${request.type})`);
  ...
  private startConfigWatcher(): void {
    ...
    this.configWatcher = createConfigWatcher(this.workspaceRootPath, {
      onValidationError: (file, result) => {
        debug('[OpenTomoAgent] Config validation error:', file, result.errors);
        this.onConfigValidationError?.(file, result.errors);
      },
      onError: (file, error) => {
        debug('[OpenTomoAgent] Config file error:', file, error.message);
      },
    });

    debug('[OpenTomoAgent] Config watcher started');
  }
  ...
  private stopConfigWatcher(): void {
    if (this.configWatcher) {
      ...
      debug('[OpenTomoAgent] Config watcher stopped');
    }
  }
  ...
  private handleSourceUpdate(slug: string, source: LoadedSource | null): void {
    if (!source) {
      ...
      debug('[OpenTomoAgent] Removed source:', slug);
      return;
    }
  ...
  setThinkingLevel(level: ThinkingLevel): void {
    ...
    this.onDebug?.(`[OpenTomoAgent] Thinking level: ${level}`);
  }
  ...
  async *chat(
    ...
    try {
      ...
      const chatSpan = perf.span('chat', { sessionId });
      ...
      const currentPreferencesPrompt = await formatUserContextForPrompt();

      if (this.pinnedContextPrompt === null) {
        // First chat in this session - pin current values
        this.pinnedContextPrompt = currentPreferencesPrompt;
        ...
        const preferencesDrifted = currentPreferencesPrompt !== this.pinnedContextPrompt;

        if (preferencesDrifted && !this.contextDriftNotified) {
          yield {
            type: 'info',
            message: `Note: Your preferences changed since this session started. Start a new session to apply changes.`,
          };
          this.contextDriftNotified = true;
          debug(`[chat] Detected drift in: preferences`);
        }
      }

      chatSpan.mark('chat.context_pinned');
      ...
      const mcpServers: Options['mcpServers'] = isMiniAgent
        ? {
            ...
            // Memory tools - episode memory (memory_save, memory_search, memory_read)
            memory: getMemoryServer(this.workspaceRootPath),
            'opentomo-docs': {
              type: 'http',
              url: 'https://github.com/OpenTomo/opentomo',
            },
          }
        : {
            // Memory tools - episode memory (memory_save, memory_search, memory_read)
            ...
            session: getSessionScopedTools(sessionId, this.workspaceRootPath),
            // OpenTomo documentation - always available for searching setup guides
            ...
            // Note: OpenTomo MCP server is now added via sources system
            ...sourceMcpResult.servers,
            ...this.sourceApiServers,
            // Generative UI guidelines server (on-demand, in-process MCP)
            ...(this.config.generativeUiEnabled !== false ? { 'opentomo-widget': getWidgetGuidelinesServer() } : {}),
          };
      
      chatSpan.mark('chat.mcp_config_built');
      ...
      if (activeBaseUrl) {
        debug(`[chat] Custom provider: baseUrl=${activeBaseUrl}, model=${model}, hasApiKey=${!!process.env.ANTHROPIC_API_KEY}, hasAuthToken=${!!process.env.ANTHROPIC_AUTH_TOKEN}`);
      }

      const thinkingTokens = getThinkingTokens(this.thinkingLevel, modelConfig);
      debug(`[chat] Thinking: level=${this.thinkingLevel}, tokens=${thinkingTokens}`);
      ...
      const isProxy = false;
      // Determine safe beta set (default none; optional context-1m via env)
      const betas = resolveAllowedBetas(isProxy, activeBaseUrl, model);
      debug(`[OpenTomoAgent] SDK betas applied: ${betas.length ? betas.join(', ') : 'none'}`);

      // Log mini agent mode details
      if (isMiniAgent) {
        debug('[OpenTomoAgent] 🤖 MINI AGENT mode - optimized for quick config edits');
        debug('[OpenTomoAgent] Mini agent optimizations:', {
          model,
          tools: ['Read', 'Edit', 'Write', 'Glob', 'Grep', 'Bash'],
          mcpServers: ['session', 'opentomo-docs'],
          thinking: 'disabled',
          systemPrompt: 'lean (no Claude Code preset)',
        });
      }

      // Build system prompt asynchronously before constructing options
      // Resolve effective agent name: workspace config > preferences > default
      const wsConfig = loadWorkspaceConfig(this.workspaceRootPath);
      const effectiveAgentName = wsConfig?.agent?.name || getAgentName(this.workspaceRootPath);

      let resolvedSystemPrompt: string | { type: 'preset'; preset: 'claude_code'; append: string };
      if (this.config.systemPromptPreset === 'mini') {
        resolvedSystemPrompt = await getSystemPrompt(undefined, undefined, this.workspaceRootPath, undefined, 'mini', effectiveAgentName);
      } else {
        // Cache system prompt after first build to avoid re-reading files on every message
        if (!this.cachedSystemPrompt) {
          this.cachedSystemPrompt = await getSystemPrompt(
            this.pinnedContextPrompt ?? undefined,
            this.config.debugMode,
            this.workspaceRootPath,
            this.config.session?.workingDirectory,
            undefined,
            effectiveAgentName,
            this.config.connectorIds,
            this.config.generativeUiEnabled
          );
        }
        chatSpan.mark('chat.system_prompt_built');
        resolvedSystemPrompt = {
          type: 'preset' as const,
          preset: 'claude_code' as const,
          append: this.cachedSystemPrompt,
        };
      }

      // Get base (sanitized) SDK options once and reuse to avoid reintroducing unsafe env vars
      ...
      const options: Options = {
        ...
        // Beta features: always set betas explicitly (even empty) to prevent SDK from attaching unsupported defaults
        betas: betas as any,
        // Extended thinking: tokens based on session-level thinking level
        ...
        // Proxy connections also disable thinking (OpenRouter doesn't forward thinking params)
        maxThinkingTokens: isMiniAgent ? 0 : (isClaude && !isProxy ? thinkingTokens : 0),
        ...
        tools: (() => {
          ...
          debug('[OpenTomoAgent] 🔧 Tools configuration:', JSON.stringify(toolsValue));
          return toolsValue;
        })(),
        ...
        hooks: {
            ...
            hooks: [async (input) => {
              ...
              if (input.tool_name.startsWith('mcp__')) {
                ...
                if (parts.length >= 3 && serverName) {
                  ...
                  // - opentomo-docs: always-available documentation search
                  // - memory: episode memory tools
                  // - opentomo-widget: generative UI guidelines (in-process, not a user source)
                  const builtInMcpServers = new Set(['session', 'opentomo-docs', 'memory', 'opentomo-widget']);
              ...
              if (input.tool_name === 'Skill') {
                ...
                // Block disabled skills before qualification
                if (toolInput.skill && this.config.disabledSkillSlugs?.length) {
                  const slug = toolInput.skill.includes(':')
                    ? toolInput.skill.split(':').pop()
                    : toolInput.skill;
                  if (slug && this.config.disabledSkillSlugs.includes(slug)) {
                    this.onDebug?.(`Skill tool: blocked disabled skill "${slug}"`);
          ...
          SubagentStart: [{
            hooks: [async (input, _hookToolUseID) => {
              ...
              debug(`[OpenTomoAgent] SubagentStart: agent_id=${typedInput.agent_id}, type=${typedInput.agent_type}`);
              return { continue: true };
            }],
          }],
          SubagentStop: [{
            hooks: [async (input, _toolUseID) => {
              ...
              debug(`[OpenTomoAgent] SubagentStop: agent_id=${typedInput.agent_id}`);
              return { continue: true };
            }],
          }],
        },
        ...
        // Inject bundled runtime paths into agent environment while preserving
        // the sanitized env from getDefaultOptions() (prevents SDK from
        // picking up beta toggles like ADVANCED_TOOL_USE from process.env).
        env: (() => {
          const runtimeEnv = buildAgentEnvironment(getGlobalRegistry());
          // Start from sanitized base env to avoid implicit beta headers/warnings,
          // then layer runtime-specific PATH and other required vars.
          const sanitizedEnv = (baseOptions as Partial<Options>).env || {};

          // Inject workspace .env vars so Skills and tools can access user-defined secrets
          // (e.g. API keys). Reserved auth-critical prefixes are blocked to prevent
          // workspace env from overriding authentication or internal framework vars.
          const RESERVED_PREFIXES = ['ANTHROPIC_', 'CLAUDE_CODE_', 'SS_', 'BUN_'];
          const workspaceEnvVars = readWorkspaceEnvFile(this.workspaceRootPath);
          const workspaceEnv: Record<string, string> = {};
          for (const { key, value } of workspaceEnvVars) {
            if (!key.trim()) continue;
            const upper = key.toUpperCase();
            if (RESERVED_PREFIXES.some(prefix => upper.startsWith(prefix))) continue;
            workspaceEnv[key] = value;
          }

          return { ...sanitizedEnv, ...workspaceEnv, ...runtimeEnv } as Record<string, string>;
        })(),
      };

      chatSpan.mark('chat.options_assembled');
      ...
      if (wasResuming) {
        console.error(`[OpenTomoAgent] Attempting to resume SDK session: ${this.sessionId}`);
        debug(`[OpenTomoAgent] Attempting to resume SDK session: ${this.sessionId}`);
      } else {
        console.error(`[OpenTomoAgent] Starting fresh SDK session (no resume)`);
        debug(`[OpenTomoAgent] Starting fresh SDK session (no resume)`);
      }
      ...
      chatSpan.mark('chat.query_called');
      ...
      // Track first SDK message and first text_delta for latency measurement
      let firstSdkMessageMarked = false;
      let firstTextDeltaMarked = false;
      try {
        for await (const message of this.currentQuery) {
          if (!firstSdkMessageMarked) {
            chatSpan.mark('chat.first_sdk_message');
            firstSdkMessageMarked = true;
          }
          ...
          for (const event of events) {
            ...
            if (!firstTextDeltaMarked && event.type === 'text_delta') {
              chatSpan.mark('chat.first_text_delta');
        ...
        if (wasResuming && !receivedAssistantContent && !_isRetry) {
          ...
          this.pinnedContextPrompt = null;
          this.cachedSystemPrompt = null;
          this.contextDriftNotified = false;
        ...
        // message_delta event that normally triggers text_complete
        ...
        console.error(`[OpenTomoAgent] INNER CATCH triggered: ${sdkError instanceof Error ? sdkError.message : String(sdkError)}`);
        ...
        if (isConfigCorruption && !_isRetry) {
          debug('[OpenTomoAgent] Detected .claude.json corruption, repairing and retrying...');
        ...
        if (isProcessError) {
          ...
          if (isSessionExpired && wasResuming && !_isRetry) {
            ...
            console.error('[OpenTomoAgent] SDK session expired server-side, clearing and retrying fresh');
            debug('[OpenTomoAgent] SDK session expired server-side, clearing and retrying fresh');
            this.sessionId = null;
      ...
      console.error(`[OpenTomoAgent] OUTER CATCH triggered: ${error instanceof Error ? error.message : String(error)}`);
      console.error(`[OpenTomoAgent] Error stack: ${error instanceof Error ? error.stack : 'no stack'}`);
  ...
  private formatSourceState(): string {
    ...
    if (inactiveSources.length > 0) {
      const inactiveList = inactiveSources.map((s) => {
        const reason = !s.config.enabled ? 'disabled' : 'inactive';
   ...
   * Local sources don't require authentication.
   */
...
`;
  ...
  /**
   * Build a provider-specific error for authentication failures.
   * Avoids hardcoding "Anthropic" in messages when using other providers.
   */
  private buildAuthFailedError(): AgentError {
    return {
      code: 'invalid_api_key',
      title: 'Authentication Failed',
      message: 'Unable to authenticate with the AI provider. Check your credentials in settings.',
      details: ['Check your API key or token in settings', 'Ensure your credentials have not been revoked'],
      actions: [
  ...
  private async mapSDKErrorToTypedError(
    ...
    // Use Partial<Record<...>> so we can define only known codes and default the rest
    const errorMap: Partial<Record<SDKAssistantMessageError, AgentError>> = {
      'authentication_failed': this.buildAuthFailedError(),
      'billing_error': {
        code: 'billing_error',
        title: 'Billing Error',
      ...
      // Optional error codes that may be present in newer SDK versions
      // Map them to user-friendly messages if encountered
      'max_output_tokens': {
        code: 'invalid_request',
        title: 'Output Limit Exceeded',
        message: 'The response exceeded the maximum output token limit. Try asking for a shorter response or reduce verbosity.',
        actions: [
          { key: 'r', label: 'Retry', action: 'retry' },
        ],
        canRetry: true,
    ...
    let error = errorMap[errorCode] ?? errorMap['unknown']!;
  ...
  private async convertSDKMessage(
    ...
    switch (message.type) {
      ...
      case 'result': {
        ...
        console.error(`[OpenTomoAgent] result message: subtype=${message.subtype}, errors=${'errors' in message ? JSON.stringify((message as any).errors) : 'none'}`);
  ...
  /**
   * Update the active connector IDs and invalidate the cached system prompt
   * if the set of connected connectors has changed.
   * Called on each message send so the system prompt stays current.
   */
  updateConnectorIds(ids: string[]): void {
    const current = JSON.stringify((this.config.connectorIds ?? []).slice().sort());
    const next = JSON.stringify(ids.slice().sort());
    if (current !== next) {
      this.config.connectorIds = ids;
      this.cachedSystemPrompt = null;
    }
  }
   ...
   * but not yet persisted to the document
   */
