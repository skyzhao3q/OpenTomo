...
/**
 * Build a sanitized environment for the SDK subprocess.
 *
 * Some SDK releases may attach beta headers based on environment variables
 * (e.g., advanced tool use betas). Our proxy does not accept these headers
 * and will emit warnings like:
 *   "Warning: Beta header 'advanced-tool-use-2025-11-20' is not allowed"
 *
 * To avoid this, strip any env keys that look like beta toggles before
 * spawning the subprocess. Callsites can still explicitly set `betas` in
 * Options; this only removes implicit env-driven betas.
 */
function buildSanitizedSubprocessEnv(): Record<string, string> {
    ...
    for (const [key, value] of Object.entries(combined)) {
        ...
        // Drop any beta-related toggles
        const isBetaLike = upper.includes('BETA') || upper.includes('ADVANCED_TOOL_USE');
        if (isBetaLike) continue;
        sanitized[key] = value;
    }

    // If routed through a custom proxy, strip direct Anthropic API key envs.
    // Otherwise the SDK may silently authenticate with the user's API key even when
    // a proxy base URL is set.
    const baseUrlRaw = combined.ANTHROPIC_BASE_URL || '';
    const baseUrl = String(baseUrlRaw).trim().toLowerCase();
    const isOpenTomoProxy = !!process.env.OPENTOMO_PROXY_ENABLED;
    if (isOpenTomoProxy) {
        // Remove direct Anthropic credentials and Foundry toggles when proxied.
        delete sanitized.ANTHROPIC_API_KEY;
        delete sanitized.CLAUDE_CODE_USE_FOUNDRY;
        delete sanitized.ANTHROPIC_FOUNDRY_API_KEY;
        delete sanitized.ANTHROPIC_FOUNDRY_RESOURCE;
    }
    debug(`[options] subprocess env: ANTHROPIC_BASE_URL=${sanitized.ANTHROPIC_BASE_URL ?? '(not set)'}, hasAuthToken=${!!sanitized.ANTHROPIC_AUTH_TOKEN}, hasApiKey=${!!sanitized.ANTHROPIC_API_KEY}`);
    return sanitized;
}
    ...
    if (customPathToClaudeCodeExecutable) {
        ...
        return {
            ...
            env: {
                ...
                SS_DEBUG: (process.argv.includes('--debug') || process.env.SS_DEBUG === '1') ? '1' : '0',
            },
            // SAFE DEFAULT: Do not send any beta headers unless a callsite explicitly opts in.
            // Some SDK releases attach advanced tool-use betas implicitly, which our proxy rejects.
            // Individual utilities can override (e.g., context-1m) as needed.
            betas: [] as unknown as any,
        };
    }

    if (typeof SS_CLI_VERSION !== 'undefined' && SS_CLI_VERSION != null) {
        const baseDir = join(homedir(), '.local', 'share', 'opentomo', 'versions', SS_CLI_VERSION);
        return {
            ...
            // SAFE DEFAULT: prevent implicit beta headers
            betas: [] as unknown as any,
        }
    }
    return {
