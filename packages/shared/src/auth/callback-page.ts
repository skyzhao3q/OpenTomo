...
import { SS_LOGO_HTML } from '../branding.ts';
...
export function generateCallbackPage(options: {
  ...
  const { title, isSuccess, errorDetail, deeplinkUrl, appName = 'OpenTomo' } = options;
...
<body>
  <div class="content">
    <pre class="logo">${SS_LOGO_HTML}</pre>
    <div class="card">
      <div class="status">${statusMessage}</div>
    </div>
    <div class="hint">You can now return to the application.</div>
    ${isSuccess && deeplinkUrl ? `<a href="${deeplinkUrl}" class="return-link">${appName}</a>` : ''}
  </div>
