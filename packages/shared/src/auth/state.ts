...
import { loadStoredConfig, getActiveWorkspace, getAuthType, type AuthType, type Workspace } from '../config/storage.ts';
...
export async function getAuthState(): Promise<AuthState> {
  ...
  const authType = getAuthType(); // Use helper that falls back to defaults

  // Determine if billing credentials are satisfied based on auth type
  let hasCredentials = false;
  if (authType === 'api_key' || authType === 'custom_api') {
    // custom_api (provider connections) and api_key share the same credential requirements:
    // either a stored API key or a custom base URL (for keyless providers like Ollama).
...
export function getSetupNeeds(state: AuthState): SetupNeeds {
  ...
  // Need a workspace if none exists yet (e.g. clean install or config.json deleted)
  const needsWorkspace = !state.workspace.hasWorkspace;

  return {
    needsBillingConfig,
    needsCredentials,
    needsWorkspace,
    isFullyConfigured: !needsBillingConfig && !needsCredentials && !needsWorkspace,
