 ...
 *
 * Storage locations:
 * - App override:   ~/.opentomo/theme.json
 * - Preset themes:  ~/.opentomo/themes/*.json
 */
...
/**
 * Core theme colors (10-color semantic system)
 */
export interface ThemeColors {
  ...
  main?: CSSColor; // メイン装飾色
  highlight?: CSSColor; // ハイライト色
  secondary?: CSSColor; // セカンダリ色
  tertiary?: CSSColor; // ターシャリ色
}
...
export function themeToCSS(theme: ThemeOverrides, isDark: boolean = false): string {
  ...
  // Extended 4 colors (main, highlight, secondary, tertiary)
  if (colors.main) {
    vars.push(`--main: ${colors.main};`);
    const rgbValues = hexToRgbValues(colors.main, 0.7);
    if (rgbValues) {
      vars.push(`--main-rgb: ${rgbValues};`);
    }
  }
  if (colors.highlight) {
    vars.push(`--highlight: ${colors.highlight};`);
    const rgbValues = hexToRgbValues(colors.highlight, 0.7);
    if (rgbValues) {
      vars.push(`--highlight-rgb: ${rgbValues};`);
    }
  }
  if (colors.secondary) {
    vars.push(`--secondary: ${colors.secondary};`);
    const rgbValues = hexToRgbValues(colors.secondary, 0.7);
    if (rgbValues) {
      vars.push(`--secondary-rgb: ${rgbValues};`);
    }
  }
  if (colors.tertiary) {
    vars.push(`--tertiary: ${colors.tertiary};`);
    const rgbValues = hexToRgbValues(colors.tertiary, 0.7);
    if (rgbValues) {
      vars.push(`--tertiary-rgb: ${rgbValues};`);
    }
  }
 ...
 * that visually match the DEFAULT_THEME colors.
 */
...
/**
 * Default theme values (GitHub Design System)
 */
