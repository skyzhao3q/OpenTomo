 ...
 *
 * Stores credentials in an encrypted file at ~/.opentomo/credentials.enc
...
// After release, this value must NEVER change — it would make all credentials.enc files unreadable.
const MAGIC_BYTES = Buffer.from('TOMO26\0\0');
...
function getStableMachineId(): string {
  const cacheFile = join(CREDENTIALS_DIR, '.machine-id');
  ...
  try {
    if (process.platform === 'darwin') {
      ...
      if (match?.[1]) {
        // Cache for future resilience (survives ioreg failures on next boot)
        try { writeFileSync(cacheFile, match[1], { mode: 0o600 }); } catch { /* ignore */ }
        return match[1];
      }
    ...
    // OS command failed — try cached value before falling back
    try {
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('opentomo-v2') // KDF salt — must NEVER change after release
      .digest();
  ...
  private getLegacyEncryptionKey(salt: Buffer): Buffer {
    const legacyMachineId = createHash('sha256')
      ...
      .update(homedir())
      .update('opentomo-v1') // KDF salt — must NEVER change after release
      .digest();
