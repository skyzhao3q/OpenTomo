...
export class CredentialManager {
  ...
  // ============================================================
  // Provider Connection Credential Methods
  // ============================================================

  /** Get the API key for a named provider connection */
  async getConnectionApiKey(connectionId: string): Promise<string | null> {
    const cred = await this.get({ type: 'connection_api_key', name: connectionId });
    return cred?.value || null;
  }

  /** Set the API key for a named provider connection */
  async setConnectionApiKey(connectionId: string, key: string): Promise<void> {
    await this.set({ type: 'connection_api_key', name: connectionId }, { value: key });
  }

  /** Delete the API key for a named provider connection */
  async deleteConnectionApiKey(connectionId: string): Promise<void> {
    await this.delete({ type: 'connection_api_key', name: connectionId });
  }

  // ============================================================
  // Media Provider Credential Methods
  // ============================================================

  /** Get the API key for a media provider (e.g., 'gemini') */
  async getMediaProviderApiKey(providerSlug: string): Promise<string | null> {
    const cred = await this.get({ type: 'media_provider_apikey', name: providerSlug });
    return cred?.value || null;
  }

  /** Set the API key for a media provider */
  async setMediaProviderApiKey(providerSlug: string, key: string): Promise<void> {
    await this.set({ type: 'media_provider_apikey', name: providerSlug }, { value: key });
  }

  /** Delete the API key for a media provider */
  async deleteMediaProviderApiKey(providerSlug: string): Promise<void> {
    await this.delete({ type: 'media_provider_apikey', name: providerSlug });
  }
