...
export type CredentialType =
  ...
  // Source credentials (stored at ~/.opentomo/workspaces/{ws}/sources/{slug}/)
  ...
  | 'connection_api_key' // API key for a named provider connection (keyed by connectionId)
  // Media provider credentials
  | 'media_provider_apikey'; // API key for a media provider (keyed by providerSlug, e.g., 'gemini')
...
/** Connection credential types (keyed by connectionId via the `name` field) */
const CONNECTION_CREDENTIAL_TYPES = [
  'connection_api_key',
] as const;

/** Media provider credential types (keyed by providerSlug via the `name` field) */
const MEDIA_CREDENTIAL_TYPES = [
  'media_provider_apikey',
] as const;

/** Check if type is a media provider credential */
function isMediaCredential(type: CredentialType): boolean {
  return (MEDIA_CREDENTIAL_TYPES as readonly string[]).includes(type);
}
...
/** Check if type is a connection credential */
function isConnectionCredential(type: CredentialType): boolean {
  return (CONNECTION_CREDENTIAL_TYPES as readonly string[]).includes(type);
}
...
export function credentialIdToAccount(id: CredentialId): string {
  ...
  // Connection-scoped format: connection_api_key::{connectionId}
  if (isConnectionCredential(id.type) && id.name) {
    parts.push(id.name);
    return parts.join(CREDENTIAL_DELIMITER);
  }

  // Media provider format: media_provider_apikey::{providerSlug}
  if (isMediaCredential(id.type) && id.name) {
    parts.push(id.name);
    return parts.join(CREDENTIAL_DELIMITER);
  }
...
export function accountToCredentialId(account: string): CredentialId | null {
  ...
  // Connection-scoped format: connection_api_key::{connectionId}
  if (isConnectionCredential(type) && parts.length === 2) {
    return { type, name: parts[1] };
  }

  // Media provider format: media_provider_apikey::{providerSlug}
  if (isMediaCredential(type) && parts.length === 2) {
    return { type, name: parts[1] };
  }
