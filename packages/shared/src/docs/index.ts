 ...
 *
 * Docs are stored at ~/.opentomo/docs/ and synced from bundled assets.
...
// Lazily loaded bundled docs (populated on first getBundledDocs call)
...
export const APP_ROOT = '~/.opentomo';
...
export function initializeDocs(): void {
  ...
  // Write all docs to user directory for agent access
  for (const [filename, content] of Object.entries(bundledDocs)) {
    const targetPath = join(DOCS_DIR, filename);
    try {
      writeFileSync(targetPath, content, 'utf-8');
      debug(`[docs] Wrote ${filename}`);
    } catch (error) {
      console.error(`[docs] Failed to write ${filename}:`, error);
    }
  }
}
...
// Re-export external links (for locale-aware public website URLs)
export {
  getExternalUrl,
  EXTERNAL_LINKS,
  type ExternalLinkId,
