#!/usr/bin/env bun
/**
 * Debug script to print the full OpenTomo system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                   OPENTOMO SYSTEM PROMPT BREAKDOWN${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. OpenTomo Environment Marker - version, platform, arch');
...
const systemPrompt = await getSystemPrompt(
  ...
  '/Users/example/.opentomo/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = await getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@opentomo/electron/main.log' },
  '/Users/example/.opentomo/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.opentomo/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by OpenTomoAgent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// OpenTomo additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. OpenTomo Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/opentomo-agent.ts       ${colors.dim}// User message building${colors.reset}
