...
class SessionPersistenceQueue {
  ...
  /** Per-session write locks to serialize concurrent writes and prevent ENOENT races on .tmp files */
   ...
   * Serialized per session to prevent concurrent writes racing on the same .tmp file.
   */
  private async write(sessionId: string): Promise<void> {
    // Serialize writes per session: wait for any in-flight write to finish first
    const prev = this.writeLocks.get(sessionId) ?? Promise.resolve()
    let resolve: () => void
    const lock = new Promise<void>(r => { resolve = r })
    this.writeLocks.set(sessionId, lock)
    await prev
