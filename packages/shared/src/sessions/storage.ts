...
export function listSessions(workspaceRootPath: string): SessionMetadata[] {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      // Only delete if older than 60 seconds to avoid racing with active writes.
      const tmpFile = jsonlFile + '.tmp';
      if (existsSync(tmpFile)) {
        try {
          const age = Date.now() - statSync(tmpFile).mtimeMs;
          if (age > 60_000) unlinkSync(tmpFile);
        } catch { /* ignore */ }
      }
 ...
 * Note: Status feature has been removed. This function is kept for backwards compatibility
 * but now just returns an empty array.
 */
