...
// ============================================================
// Bundled Runtime & Skills Paths
// ============================================================

/**
 * Resolve the path to the bundled builtin skills directory.
 *
 * Tries candidates in order:
 * 1. Electron build output: <assetsRoot>/resources/skills/builtin
 *    (assetsRoot = __dirname = dist/, resources are copied to dist/resources/ by build)
 * 2. Dev fallback: apps/electron/resources/skills/builtin (source location)
 *
 * Returns the first candidate that exists on disk, or undefined if none found.
 */
export function getBundledSkillsDir(): string | undefined {
  const candidates = [
    // Electron build output: dist/resources/skills/builtin
    ...(_assetsRoot ? [join(_assetsRoot, 'resources', 'skills', 'builtin')] : []),
    // Dev: Electron resources in monorepo (source)
    join(process.cwd(), 'apps', 'electron', 'resources', 'skills', 'builtin'),
  ];
  return candidates.find(p => existsSync(p));
}
