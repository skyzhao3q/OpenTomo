...
export async function installArchive(params: { archiveData: ArrayBuffer, version: string }): Promise<void> {
  ...
  const versionDirectory = join(homedir(), '.local', 'share', 'opentomo', 'versions', version);
  const binaryPath = join(versionDirectory, 'opentomo');
  const symlinkDirectory = join(homedir(), '.local', 'bin');
  const symlinkPath = join(symlinkDirectory, 'opentomo');
