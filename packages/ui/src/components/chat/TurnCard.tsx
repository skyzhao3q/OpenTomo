...
import type { ToolDisplayMeta } from '@opentomo/core'
import { normalizePath, pathStartsWith, stripPathPrefix } from '@opentomo/core/utils'
...
export interface TurnCardProps {
  ...
  /** Optional agent icon displayed at the top of each turn (e.g., <AgentAvatar />) */
...
export function ResponseCard({
  ...
  // Widget-containing responses need unconstrained height for iframe rendering
  ...
  if (isCompleted || variant === 'plan') {
    ...
    return (
      <>
        <div className="bg-background shadow-minimal rounded-[8px] overflow-x-auto overflow-y-hidden relative group">
          ...
          {/* Widget responses get unconstrained height — iframes need full space to render charts */}
          <div
            className="pl-[22px] pr-[16px] py-3 text-sm overflow-y-auto"
            style={{
              ...(!hasWidget && { maxHeight: MAX_HEIGHT }),
              // Subtle fade at top and bottom edges (16px) - only in dark mode for better contrast
              ...(!hasWidget && isDarkMode && {
  ...
  return (
    <div className="bg-background shadow-minimal rounded-[8px] overflow-x-auto overflow-y-hidden">
      {/* Content area - uses displayedText (throttled) for performance */}
      {/* No maxHeight during streaming so content grows freely and ChatDisplay's */}
      {/* ResizeObserver can auto-scroll the outer viewport */}
      <div
...
export const TurnCard = React.memo(function TurnCard({
  ...
  return (
    <div className="space-y-1">
      {/* Agent icon header - shown at top of every turn for visual identity */}
      {agentIcon && (
        <div className="flex items-center gap-1.5 px-2.5 pt-0.5 pb-0.5">
          {agentIcon}
        </div>
      )}
              ...
              >
                ...
                >
                  ...
                  {isThinking && !animateResponse && (
                    ...
                    >
                      <Spinner className={cn(SIZE_CONFIG.spinnerSize, "text-accent-cool")} />
