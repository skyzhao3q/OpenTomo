...
import type { StoredAttachment, ContentBadge } from '@opentomo/core'
import { normalizePath } from '@opentomo/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .opentomo workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.opentomo\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
