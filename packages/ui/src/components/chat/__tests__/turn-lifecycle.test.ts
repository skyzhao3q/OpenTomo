...
import type { Message } from '@opentomo/core'
...
describe('streaming pending message promotion', () => {
  it('last pending message becomes turn.response', () => {
    resetCounters()
    turnIdCounter++

    // Tool completes, then a pending (streaming) assistant message arrives
    const messages: Message[] = [
      createUserMessage(),
      {
        ...
        toolStatus: 'completed',
        toolResult: 'File contents',
        turnId: `turn-${turnIdCounter}`,
      },
      {
        id: 'pending-msg',
        role: 'assistant',
        content: 'Streaming response with ```opentomo-widget\n{}\n```',
    ...
    // The last pending message should be promoted to response
    expect(assistantTurn.response).toBeDefined()
    expect(assistantTurn.response!.isStreaming).toBe(true)
    expect(assistantTurn.response!.text).toContain('opentomo-widget')
  })

  it('non-last pending messages stay as activity', () => {
    resetCounters()
    turnIdCounter++

    // Two pending messages — only the last should become response
    const messages: Message[] = [
      createUserMessage(),
      {
        id: 'pending-1',
        role: 'assistant',
        content: 'First pending text',
        timestamp: Date.now() + 200,
        isPending: true,
        isStreaming: false,
        isIntermediate: false,
      ...
      },
      {
        id: 'pending-2',
        role: 'assistant',
        content: 'Second pending text (streaming)',
        timestamp: Date.now() + 300,
        isPending: true,
        isStreaming: true,
        turnId: `turn-${turnIdCounter}`,
    ...
    // First pending should be an activity
    const activityContents = assistantTurn.activities.map(a => a.content)
    expect(activityContents).toContain('First pending text')

    // Second (last) pending should be the response
    expect(assistantTurn.response).toBeDefined()
    expect(assistantTurn.response!.text).toBe('Second pending text (streaming)')
  })
})
