...
import type { Message, StoredMessage, MessageRole } from '@opentomo/core'
...
export function groupMessagesByTurn(messages: Message[]): Turn[] {
  ...
  const lastSortedMessage = sortedMessages[sortedMessages.length - 1]
  ...
  const flushCurrentTurn = (interrupted = false) => {
    if (currentTurn) {
      ...
      // If no response but we have intermediate text, promote to response
      ...
      if (!interrupted && !currentTurn.response && currentTurn.isComplete && currentTurn.activities.length > 0) {
        // Combine all top-level intermediate text activities into the response
        const intermediateTexts = currentTurn.activities
          .filter(a => a.type === 'intermediate' && !a.parentId && a.content?.trim())
          .map(a => a.content!)

        if (intermediateTexts.length > 0) {
          currentTurn.response = {
            text: intermediateTexts.join('\n\n'),
            isStreaming: false,
          }
        }
      }
  ...
  for (const message of sortedMessages) {
    ...
    if (message.role === 'assistant') {
      ...
      if (message.isIntermediate || (message.isPending && message !== lastSortedMessage)) {
        if (!currentTurn) {
      ...
      // Collect intermediate text to prepend to the response.
      // When the SDK splits output into multiple text blocks (e.g. text → tool_call → text),
      // earlier blocks arrive as isIntermediate=true and get classified as activities.
      // Without this, only the final text block becomes the response and earlier content is lost.
      const intermediateTexts = currentTurn.activities
        .filter(a => a.type === 'intermediate' && !a.parentId && a.content?.trim())
        .map(a => a.content!)

      // Set as response on current turn (ignoring turnId differences)
      const fullText = intermediateTexts.length > 0
