...
registerCustomTheme('ss-dark', async () => {
  const theme = await resolveTheme('pierre-dark')
  return { ...theme, name: 'ss-dark', bg: 'transparent', colors: { ...theme.colors, 'editor.background': 'transparent' } }
})
registerCustomTheme('ss-light', async () => {
  const theme = await resolveTheme('pierre-light')
  return { ...theme, name: 'ss-light', bg: 'transparent', colors: { ...theme.colors, 'editor.background': 'transparent' } }
})
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to ss-dark/ss-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to ss-dark/ss-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'ss-dark' : 'ss-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__ssClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__ssClickCleanup?.()
      }
    }
