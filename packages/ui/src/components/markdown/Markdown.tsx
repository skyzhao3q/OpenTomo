...
import { ImageGenConfirmBlock, ImageGenResultBlock } from './MarkdownImageGenBlock'
    ...
    return {
      ...
      code: ({ className, children, ...props }) => {
        ...
        if (match || isBlock) {
          ...
          // Image generation blocks → interactive cards
          if (match?.[1] === 'image-gen-request') {
            return <ImageGenConfirmBlock code={code} className="my-1" />
          }
          if (match?.[1] === 'image-gen-result') {
            return <ImageGenResultBlock code={code} className="my-1" />
          }
          // Generative UI widget blocks → sandbox iframe renderer
          if (match?.[1] === 'show-widget') {
            try {
              const json = JSON.parse(code)
              if (json.widget_code) {
                return <WidgetRenderer widgetCode={String(json.widget_code)} isStreaming={false} title={json.title} />
              }
            } catch {
              // malformed — fall through to CodeBlock
            }
          }
