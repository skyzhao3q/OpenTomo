/**
 * SVGPreviewOverlay — fullscreen SVG preview with zoom/pan and code view toggle.
 *
 * Renders SVG files with vector-quality zoom controls similar to Mermaid preview,
 * plus a toggle to view the raw SVG markup with syntax highlighting.
 *
 * Features:
 *   - Toggle between rendered preview and syntax-highlighted code view
 *   - Zoom/pan controls in preview mode (mousewheel, trackpad pinch, drag)
 *   - Copy button (copies raw SVG markup)
 *   - Keyboard shortcuts (Cmd+/-, Cmd+0, Escape)
 *
 * Input methods (preview mode):
 ...
 *
 * Header controls: [View Code toggle] [−] [▾ 100%] [+] [⟲] [copy]
 */

import { useState, useRef, useEffect, useCallback } from 'react'
import { Image, RotateCcw, Minus, Plus, Check, Code2, Eye } from 'lucide-react'
...
export interface SVGPreviewOverlayProps {
  isOpen: boolean
  onClose: () => void
  filePath: string
  /** Raw SVG content (XML string) */
  svg: string
  /** Optional error if SVG failed to load */
  error?: string
}
...
export function SVGPreviewOverlay({
  ...
  // Reset zoom/pan and view mode when overlay opens
  useEffect(() => {
    if (isOpen) {
      setViewMode('preview')
      setScale(1)
  ...
  /** Calculate and apply the scale needed to fit the entire SVG in the viewport. */
  const zoomToFit = useCallback(() => {
    ...
    // 90% of available space so the SVG doesn't touch the edges
  ...
  useEffect(() => {
    if (viewMode !== 'preview') return // Only active in preview mode
  ...
  // Only active when overlay is open and in preview mode. preventDefault() blocks browser zoom.
  useEffect(() => {
    if (!isOpen || viewMode !== 'preview') return
  ...
  }, [isOpen, viewMode, zoomByStep, handleReset])
  ...
  const headerActions = (
    <div className="flex items-center gap-1.5">
      ...
      {/* Copy SVG button (always visible) */}
      <CopyButton
        content={svg}
        title="Copy SVG"
  ...
  return (
    <PreviewOverlay
      ...
      className={viewMode === 'code' ? 'bg-foreground-3' : undefined}
    >
      {viewMode === 'preview' ? (
        /* ── Preview mode: Zoom/pan viewport ── */
        <div
          ref={containerRef}
        ...
        /* ── Code mode: Syntax-highlighted SVG markup ── */
        <ContentFrame title="SVG Code" fitContent minWidth={850}>
          <div className="p-4">
            <CodeBlock code={svg} language="xml" mode="full" />
          </div>
        </ContentFrame>
      )}
    </PreviewOverlay>
