...
export type FilePreviewType = 'image' | 'code' | 'markdown' | 'json' | 'text' | 'pdf' | 'svg'
 ...
 * SVG is excluded — handled separately by SVGPreviewOverlay.
 */
const IMAGE_EXTENSIONS = new Set([
  'png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'ico', 'avif',
])

/** SVG files — rendered in SVGPreviewOverlay with zoom/pan and code view */
 ...
 *
 * Priority order when an extension matches multiple sets:
 * svg > image > code > markdown > json > text > pdf
 */
export function classifyFile(filePath: string): FileClassification {
  const ext = getExtension(filePath)
  if (!ext) return { type: null, canPreview: false }

  if (SVG_EXTENSIONS.has(ext))      return { type: 'svg', canPreview: true }
